...
export default class LicenseAdjustment extends LicenseAdjustmentModel {
  ...
  /**
   * Exports master adjustment items with revision information.
   */
   ...
   */
  static _load(identifier: any, byGuid: boolean = false): Promise<LicenseAdjustment | null> {
   ...
   */
  static async _updatePaymentStatus(
    id: number,
  ...
  static calculateAdjustmentAmounts(
    ...
    const subtotal_usd =
      Math.round(employees_added * months_remaining * price_per_employee_usd * 100) / 100;
   ...
   */
  async updatePaymentStatusForThis(
    status: PaymentTransactionStatus,
    completed_at?: Date,
  ): Promise<boolean> {
   ...
   */
  async load(identifier: any, byGuid: boolean = false): Promise<LicenseAdjustment | null> {
