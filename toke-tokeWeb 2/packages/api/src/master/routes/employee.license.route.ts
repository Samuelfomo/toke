...
router.get('/:guid', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    console.error('❌ Error retrieving employee master:', error);
    R.handleError(res, HttpStatus.INTERNAL_ERROR, {
      code: EMPLOYEE_LICENSE_CODES.SEARCH_FAILED,
      message: 'Failed to retrieve employee master',
      details: error.message,
    });
  }
});
...
// Update employee master by GUID
...
// Delete employee master by GUID
...
// Get employee master by employee
router.get('/employee/:employee', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    console.error('❌ Error retrieving employee master by employee ID:', error);
    R.handleError(res, HttpStatus.INTERNAL_ERROR, {
      code: EMPLOYEE_LICENSE_CODES.SEARCH_FAILED,
      message: 'Failed to retrieve employee master',
      details: error.message,
...
// Get employee master by employee code
router.get('/code/:employeeCode', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    console.error('❌ Error retrieving employee master by employee code:', error);
    R.handleError(res, HttpStatus.INTERNAL_ERROR, {
      code: EMPLOYEE_LICENSE_CODES.SEARCH_FAILED,
      message: 'Failed to retrieve employee master',
      details: error.message,
...
// List employee licenses by global master
...
// Get billing statistics by global master
...
router.get('/long-leave/list', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const items = await Promise.all(employeesOnLeave.map((license) => license.toJSON()));
...
router.get('/grace-period/list', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const items = await Promise.all(employeesInGrace.map((license) => license.toJSON()));
