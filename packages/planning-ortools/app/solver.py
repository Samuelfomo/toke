...
class OrToolsPlanner:
    def __init__(self, request: PlanningSolverInput):
        ...
        # Boundary continuations are fixed facts loaded from the approved planning
        # immediately preceding the technical solve horizon.
        self.boundary_guard_continuations = {
            (item.employeeGuid, item.continuationDate): item
            for item in request.boundaryState.guardContinuations
        }

        # Calendar-day state is explicit: a continuation is worked, while a
        # post-guard recovery is an authorised non-working exception.
        self.guard_continuation_day: dict[tuple[str, str], cp_model.IntVar] = {}
        self.post_guard_rest_day: dict[tuple[str, str], cp_model.IntVar] = {}
    ...
    def _validate_boundary_state(self) -> None:
        rotating_guids = {employee.guid for employee in self.rotating}
        seen: set[tuple[str, str]] = set()

        for item in self.request.boundaryState.guardContinuations:
            key = (item.employeeGuid, item.continuationDate)
            if key in seen:
                raise ValueError(
                    f"Duplicate boundary guard continuation for {item.employeeGuid} on {item.continuationDate}"
                )
            seen.add(key)

            if item.employeeGuid not in rotating_guids:
                raise ValueError(
                    f"Boundary guard continuation employee {item.employeeGuid} must have ROTATING mode"
                )
            if item.continuationDate != self.request.periodFrom:
                raise ValueError(
                    "Boundary guard continuations must target the first technical solve date"
                )
            if item.guardDate != add_days(item.continuationDate, -1):
                raise ValueError(
                    "Boundary guard continuation guardDate must be the previous calendar day"
                )
            if not template_has_work(
                item.continuationTemplate, item.continuationDate
            ):
                raise ValueError(
                    f"Boundary continuation template {item.continuationTemplate.guid} has no work on {item.continuationDate}"
                )
    ...
    def _build_guard_continuation_days(self) -> None:
        for employee in self.rotating:
            for iso in self.dates:
                ...
                boundary = (employee.guid, iso) in self.boundary_guard_continuations
                active_terms = previous_guard_terms + ([1] if boundary else [])

                variable = self.model.NewBoolVar(
                    f"guard_continuation_day_{employee.guid}_{iso}"
                )
                self.guard_continuation_day[(employee.guid, iso)] = variable
                ...
                if not active_terms:
                    ...
                    for term in active_terms:
                        self.model.Add(variable >= term)

    def _build_post_guard_rest_days(self) -> None:
        days = (
            self.request.config.postGuardRestDays
            if self.request.config.restAfterGuardRequired
            else 0
        ...
        for employee in self.rotating:
            for iso in self.dates:
                ...
                for offset in range(1, days + 1):
                    ...
                    boundary = self.boundary_guard_continuations.get(
                        (employee.guid, add_days(iso, -offset))
                    )
                    if boundary is not None:
                        causes.append(1)

                variable = self.model.NewBoolVar(
                    f"post_guard_rest_day_{employee.guid}_{iso}"
                )
                self.post_guard_rest_day[(employee.guid, iso)] = variable

                if not causes:
                    self.model.Add(variable == 0)
                else:
                    self.model.Add(variable <= sum(causes))
                    for cause in causes:
                        self.model.Add(variable >= cause)

    def _apply_guard_continuations(self) -> None:
        # Both a guard continuation and a configured full recovery day block
        # every ordinary assignment on that calendar date. This also applies to
        ...
        for employee in self.rotating:
            for iso in self.dates:
                ...
                continuation = self.guard_continuation_day[(employee.guid, iso)]
                recovery = self.post_guard_rest_day[(employee.guid, iso)]
                self.model.Add(sum(assignments) + continuation + recovery <= 1)
    ...
    def _apply_weekly_leave_policy(self) -> None:
        ...
        for employee in self.included:
            for iso in self.dates:
                ...
                if employee.mode == "ROTATING":
                    continuation = self.guard_continuation_day[(employee.guid, iso)]
                    self.model.Add(leave + continuation <= 1)

                    if not policy.postGuardRestCountsAsLeave:
                        recovery = self.post_guard_rest_day[(employee.guid, iso)]
                        self.model.Add(leave + recovery <= 1)
            ...
            for employee in self.included:
                ...
                if policy.requireWorkOnOtherDays:
                    for iso in dates:
                        ...
                        continuation = (
                            self.guard_continuation_day[(employee.guid, iso)]
                            if employee.mode == "ROTATING"
                            else 0
                        )
                        recovery = (
                            self.post_guard_rest_day[(employee.guid, iso)]
                            if employee.mode == "ROTATING"
                            else 0
                        )
                        self.model.Add(
                            scoped_work + continuation + recovery + leave >= eligible
                        )
        ...
        for employee in self.rotating:
            ...
            for _, dates in self._week_groups().items():
                ...
                boundary_minutes = sum(
                    ...
                    and item.continuationDate in date_set
                )

                if boundary_minutes > maximum:
                    raise ValueError(
    ...
    def _log_pre_solve_diagnostics(self) -> None:
        """Temporary Step-16 diagnostic trace. Does not add or alter constraints."""
        guard_policy = self.request.config.guardTeamPolicy
        leave_policy = self.request.config.weeklyLeavePolicy

        logger.info(
            "[DIAG] PERIOD requested=%s..%s solve=%s..%s dates=%d",
            self.request.requestedPeriodFrom or self.request.periodFrom,
            self.request.requestedPeriodTo or self.request.periodTo,
            self.request.periodFrom,
            self.request.periodTo,
            len(self.dates),
        )
        logger.info(
            "[DIAG] EMPLOYEES included=%d rotating=%d fixed=%d excluded=%d rotatingGuids=%s",
            len(self.included), len(self.rotating), len(self.fixed),
            len(self.request.employees) - len(self.included),
            [employee.guid for employee in self.rotating],
        )
        logger.info(
            "[DIAG] GUARD_POOL_POLICY mode=%s employeesPerWeek=%s selectionMode=%s requireParticipation=%s memberServiceAccess=%s balanceMode=%s spread=%s maxConsecutiveWeeks=%s",
            guard_policy.mode, guard_policy.employeesPerWeek,
            guard_policy.selectionMode, guard_policy.requireParticipation,
            guard_policy.memberServiceAccess, guard_policy.balance.mode,
            guard_policy.balance.maxMembershipSpread,
            guard_policy.balance.maxConsecutiveMembershipWeeks,
        )
        logger.info(
            "[DIAG] WEEKLY_LEAVE_POLICY mode=%s selectorModes=%s poolRelation=%s daysPerEmployee=%s countMode=%s allowedDays=%s maxEmployeesPerDay=%s requireWorkOnOtherDays=%s scopeMode=%s scopeTemplates=%s exclusive=%s",
            leave_policy.mode, leave_policy.selector.planningModes,
            leave_policy.selector.guardPoolRelation, leave_policy.daysPerEmployee,
            leave_policy.countMode, leave_policy.allowedDays,
            leave_policy.maxEmployeesPerDay, leave_policy.requireWorkOnOtherDays,
            leave_policy.serviceScope.mode, leave_policy.serviceScope.templateGuids,
            leave_policy.serviceScope.exclusive,
        )

        for item in self.request.boundaryState.guardContinuations:
            logger.info(
                "[DIAG] BOUNDARY_CONTINUATION employee=%s guardDate=%s continuationDate=%s template=%s creditedMinutes=%s",
                item.employeeGuid, item.guardDate, item.continuationDate,
                item.continuationTemplate.guid, item.creditedMinutes,
            )

        for iso in self.dates:
            slots = self._daily_slots(iso)
            details = []
            standard_required = 0
            guard_required = 0
            for slot in slots:
                ...
                details.append({
                    ...
                    "template": req.template.guid,
                    "allocation": req.allocationMode,
                    "min": req.minEmployees,
                    "target": req.targetEmployees,
                    "max": req.maxEmployees,
                    "eligibilityModes": req.eligibility.planningModes,
                    "poolRelation": req.eligibility.guardPoolRelation,
                })
                if req.serviceType == "GUARD":
                    guard_required += req.minEmployees
                else:
                    standard_required += req.minEmployees
            logger.info(
                "[DIAG] REQUIREMENTS_BY_DAY date=%s day=%s standardMin=%d guardMin=%d slots=%s",
                iso, day_key(iso), standard_required, guard_required, details,
            )

        for week_monday, dates in self._week_groups().items():
            pool_vars = [
                self.weekly_guard_pool.get((employee.guid, week_monday))
                for employee in self.rotating
            ]
            logger.info(
                "[DIAG] WEEK week=%s dates=%s full=%s poolVars=%d guardSlots=%d",
                week_monday, dates, len(dates) == 7,
                sum(1 for item in pool_vars if item is not None),
                sum(1 for slot in self.slots if slot.iso in dates and slot.requirement.serviceType == "GUARD"),
            )

        # Structural capacity warning for the Plateau model: with GUARD_ONLY,
        # weekly pool members cannot cover STANDARD requirements.
        if guard_policy.mode == "WEEKLY_POOL" and guard_policy.memberServiceAccess == "GUARD_ONLY":
            rotating_count = len(self.rotating)
            non_member_capacity = rotating_count - guard_policy.employeesPerWeek
            for iso in self.dates:
                standard_min = sum(
                    slot.requirement.minEmployees
                    for slot in self._daily_slots(iso)
                    if slot.requirement.serviceType == "STANDARD"
                    and "ROTATING" in slot.requirement.eligibility.planningModes
                )
                fixed_standard = sum(
                    self.fixed_counts[(slot.iso, slot.requirement.template.guid)]
                    for slot in self._daily_slots(iso)
                    if slot.requirement.serviceType == "STANDARD"
                    and "FIXED" in slot.requirement.eligibility.planningModes
                    and slot.requirement.eligibility.guardPoolRelation != "MEMBER"
                )
                if standard_min > non_member_capacity + fixed_standard:
                    logger.warning(
                        "[DIAG] POTENTIAL_CONTRADICTION date=%s reason=STANDARD_MIN_EXCEEDS_NON_GUARD_CAPACITY standardMin=%d rotatingNonMemberCapacity=%d fixedMatchingCapacity=%d totalPotential=%d",
                        iso, standard_min, non_member_capacity, fixed_standard,
                        non_member_capacity + fixed_standard,
                    )
    ...
    def _build_result(self) -> EngineResult:
        ...
        # Import the continuation already started before solveFrom. It is a fixed
        # worked state, not a new guard start and not a weekly leave.
        for boundary in self.request.boundaryState.guardContinuations:
            if boundary.employeeGuid not in schedules:
                continue
            if boundary.continuationDate not in self.dates:
                continue

            schedules[boundary.employeeGuid][
                boundary.continuationDate
            ] = boundary.continuationTemplate.guid
            reasons[boundary.employeeGuid][
                boundary.continuationDate
            ] = DayReason(
                templateName=boundary.continuationTemplate.name,
                templateGuid=boundary.continuationTemplate.guid,
                confidence=100,
                source="GUARD_CONTINUATION",
                factors=[
                    f"Suite de la garde validée commencée le {boundary.guardDate}",
                    "État de frontière chargé avant la période technique",
                    "Cette continuation compte comme travail et interdit un autre service",
                ],
            )
            ...
            if self.request.config.restAfterGuardRequired:
                for offset in range(
                    ...
                    rest_date = add_days(boundary.continuationDate, offset)
                    if rest_date not in self.dates:
                        continue
                    schedules[boundary.employeeGuid][rest_date] = None
                    reasons[boundary.employeeGuid][rest_date] = DayReason(
                        templateName="Repos post-garde",
                        templateGuid=None,
                        confidence=100,
                        source="POST_GUARD_REST",
                        factors=[
                            f"Repos après la garde validée commencée le {boundary.guardDate}",
