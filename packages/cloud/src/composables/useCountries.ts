...
export function useCountries() {
    ...
    const currentPage = computed(() => Math.floor(state.pagination.offset / state.pagination.limit) + 1);
    ...
    const fetchCountryById = async (identifier: string | number) => {
        ...
        try {
            ...
            const errorMessage = error instanceof ApiError
                ? error.getUserMessage()
                : 'Erreur lors du chargement du pays';
     ...
     */
    const updateCountry = async (guid: number, countryData: UpdateCountryDto): Promise<Country | null> => {
        setLoading(true);
        clearError();
    ...
    const applyFilters = async (filters: CountryFilters, resetPagination = true) => {
        ...
        try {
            const response = await countriesService.getFiltered(
                state.filters,
                {
