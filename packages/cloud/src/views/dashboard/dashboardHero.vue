...
/** Résumé lisible de la période active, toujours à jour — sert de confirmation visuelle */
const periodLabel = computed(() => {
  if (selectedPreset.value === 'day') return "Aujourd'hui"
  if (selectedPreset.value === 'week') return '7 derniers jours'
  if (customStartDate.value && customEndDate.value) {
    return customStartDate.value === customEndDate.value
        ? fmtDate(customStartDate.value)
        : `${fmtDate(customStartDate.value)} → ${fmtDate(customEndDate.value)}`
  }
...
const selectPreset = (preset: 'day' | 'week' | 'custom') => {
  ...
  // 'custom' : ne déclenche rien tant que l'utilisateur n'a pas choisi les deux dates
}
...
const detectPresetFromDates = (start?: string, end?: string) => {
  ...
  if (start === weekStartStr && end === today) return 'week'

  return 'custom'
}

// ─── Synchro avec le parent (une seule source, mise à jour atomique) ──────────
watch(
    () => [props.activeStartDate, props.activeEndDate] as const,
    ([start, end]) => {
  ...
  <section class="flex flex-col gap-6 py-5">
    ...
    <div class="flex flex-col md:flex-row items-center gap-4 bg-white shadow-md rounded-md
      ...
      <div class="flex items-center gap-3 shrink-0">
        ...
        <span
            class="text-xs font-semibold text-blue-600 bg-blue-50 border
             border-blue-100 px-2.5 py-1 rounded-full"
        >
      ...
      <!-- Chips + badge période active -->
      <div class="flex items-center gap-2 flex-wrap">
        <button @click="selectPreset('day')" :class="chipClass('day')">📅 Jour</button>
        <button @click="selectPreset('week')" :class="chipClass('week')">📊 Semaine</button>
        <button @click="selectPreset('custom')" :class="chipClass('custom')">🗓️ Période</button>

        <!-- Badge de confirmation : toujours visible, reflète l'état réel -->
        <span
            class="flex items-center gap-1.5 text-xs font-semibold text-slate-600
                   bg-slate-50 border border-slate-200 px-2.5 py-1.5 rounded-xl"
        >
          {{ periodLabel }}
          <button
        ...
        <!-- Loader : reflète le vrai chargement du parent -->
        <div v-if="loading" class="flex items-center gap-1 pl-1">
...
<!--    &lt;!&ndash; ═══════════════════════ HEADER + FILTRES ═══════════════════════ &ndash;&gt;-->
<!--    <div class="flex flex-col  md:flex-row items-center gap-4 bg-white shadow-md rounded-md-->
<!--            px-5 py-3.5">-->

<!--      &lt;!&ndash; Logo + titre + badge &ndash;&gt;-->
<!--      <div class="flex items-center gap-3 shrink-0">-->
<!--        <div class="w-9 h-9 bg-blue-50 rounded-xl flex items-center justify-center shrink-0">-->
<!--          <svg viewBox="0 0 24 24" fill="blue" class="w-5 h-5">-->
<!--            <rect x="2" y="12" width="4" height="10" rx="1"/>-->
...
<!--      &lt;!&ndash; Séparateur &ndash;&gt;-->
<!--      <div class="hidden md:block w-px h-8 bg-slate-200 shrink-0"></div>-->

<!--      <div class="flex items-center gap-2">-->

<!--        <div class=" bg-blue-50 rounded-xl flex-col items-end justify-start hidden">-->
...
<!--          <span-->
<!--              class="text-[9px] font-semibold text-blue-600 bg-blue-50 border-->
<!--             border-blue-100 px-2.5 rounded-full absolute"-->
<!--          >-->
...
<!--      &lt;!&ndash; Picker dates (dropdown inline) &ndash;&gt;-->
<!--      <transition name="slide-fade">-->
<!--        <div-->
...
<!--        >-->
<!--          &lt;!&ndash; Bouton Aujourd'hui &ndash;&gt;-->
<!--          <button-->
...
<!--    &lt;!&ndash; ═══════════════════════ KPI CARDS (6) ═══════════════════════ &ndash;&gt;-->
<!--    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">-->

<!--      &lt;!&ndash; 1 · Taux de présence &ndash;&gt;-->
<!--      <div class="relative flex flex-col gap-3 bg-[#F5FCF8] border border-slate-200 rounded-md p-4-->
<!--              shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">-->
<!--        <div class="flex items-start justify-between">-->
<!--          <div class="flex items-center gap-3">-->
...
<!--        </div>-->
<!--      </div>-->
<!--      &lt;!&ndash; 4 · Présences hors planning &ndash;&gt;-->
<!--      <div-->
<!--          class="relative flex flex-col gap-3 bg-[#FAFAFA] rounded-md p-4 shadow-sm-->
<!--           hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"-->
<!--          :class="summary.total_anomaly_off_days > 0-->
...
<!--      &lt;!&ndash; 5 · Couverture équipe &ndash;&gt;-->
<!--      <div class="relative flex flex-col gap-3 bg-[radial-gradient(circle,_rgba(215,230,245,1)_100%,_rgba(252,252,252,1)_100%,_rgba(215,230,245,1)_100%)] border border-slate-200 rounded-md p-4-->
<!--              shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">-->
<!--        <div class="flex items-start justify-between">-->
<!--          <div class="flex items-center gap-3">-->
...
<!--      &lt;!&ndash; 6 · Heures travaillées &ndash;&gt;-->
<!--      <div class="relative flex flex-col gap-3 bg-[radial-gradient(circle,_rgba(248,245,250,1)_100%,_rgba(252,252,252,1)_100%,_rgba(248,245,250,1)_100%)] border border-slate-200 rounded-md p-4-->
<!--              shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">-->
<!--        <div class="flex items-start justify-between">-->
<!--          <div class="flex items-center gap-3">-->
...
<!--        &lt;!&ndash; Avatars des employés concernés &ndash;&gt;-->
<!--        <div class="flex items-center gap-2 flex-wrap">-->
<!--          <div-->
<!--              v-for="occ in (summary.unexpected_presence?.occurrences ?? []).slice(0, 3)"-->
<!--              :key="occ.employee_guid + occ.date"-->
...
<!--  showPeriodPicker.value = !showPeriodPicker.value-->
<!--}-->
