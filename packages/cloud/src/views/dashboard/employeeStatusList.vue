<template>
  ...
  <div v-if="isModalOpen && selectedEmployee" class="modal-overlay" @click.self="closeModal">
    <AssiduteDuJour :employee="selectedEmployee" @close="closeModal" class="quick-detail-modal" />
  </div>
</template>
...
const employees = computed<EmployeeDisplay[]>(() => {
  ...
  return allEmployees.map((emp) => {
    ...
    console.log(
      `Employee ${emp.name} (${emp.id}): status=${dailyStatus.status}, record=`,
      todayRecord,
    );
    ...
    return {
      ...
      lateDuration,
      absenceReason:
        todayRecord?.reason || (dailyStatus.status === 'absent' ? 'Non contacté' : undefined),
...
const problemEmployees = computed(() => {
  const problems = employees.value.filter(
    (emp) => emp.status === 'absent' || emp.status === 'late',
  );
