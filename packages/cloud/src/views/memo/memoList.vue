...
/**
 * Retourne un libellé "De → Vers" pour un mémo.
 * - Mémo auto-généré : "Système → Nom du destinataire"
 * - Mémo standard   : "Nom du créateur → Nom du destinataire"
 */
const getMemoDirection = (memo: any): string => {
  const expediteur = memo.autoGenerated ? '🤖 Système' : memo.createurNom;
  const destinataire = memo.destinataireNom && memo.destinataireNom !== 'N/A'
      ? memo.destinataireNom
      : '—';
...
const memosFiltres = computed(() => {
  ...
  // Enrichir chaque mémo avec sa direction d'affichage
  return result.map(m => ({
    ...m,
    direction: getMemoDirection(m),
    expediteur: m.autoGenerated ? '🤖 Système' : m.createurNom,
    expediteurCode: m.autoGenerated ? 'SYS' : m.createurCode,
    destinataireNomAffichage: m.destinataireNom !== 'N/A' ? m.destinataireNom : '—',
    destinataireCodeAffichage: m.destinataireCode !== 'N/A' ? m.destinataireCode : '',
  }));
});
