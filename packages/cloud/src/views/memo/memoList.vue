...
/**
 * Retourne l'interlocuteur du mémo : la personne concernée côté employé.
 *
 * Règles :
 *  - Mémo auto-généré          → destinataire (l'employé pointé par le système)
 *  - Manager est le créateur   → destinataire (l'employé à qui on envoie le mémo)
 *  - Manager est le destinataire → créateur   (l'employé qui a soumis le mémo)
 *  - Fallback                  → destinataire si dispo, sinon créateur
 */
const getInterlocuteur = (memo: any): { nom: string; code: string; departement: string } => {
  ...
  // Mémo auto-généré → l'interlocuteur est le destinataire (employé concerné)
  if (memo.autoGenerated) {
    return {
  ...
  // Manager = créateur → l'interlocuteur est le destinataire
  if (memo.createurId === managerGuid.value) {
    return {
      nom: destNom ?? '—',
  ...
  // Manager = destinataire (ou rôle inconnu) → l'interlocuteur est le créateur (employé)
  return {
    nom: creatNom ?? '—',
    code: memo.createurCode ?? '',
    departement: memo.createurDepartement ?? '',
  };
};
...
const memosFiltres = computed(() => {
  ...
  // Enrichir chaque mémo avec l'interlocuteur calculé
  return result.map(m => {
    const interlocuteur = getInterlocuteur(m);
    return {
      ...m,
      interlocuteurNom: interlocuteur.nom,
      interlocuteurCode: interlocuteur.code,
      interlocuteurDepartement: interlocuteur.departement,
    };
  });
});
