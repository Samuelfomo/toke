<template>
  <Teleport to="body">
    ...
    <div class="fixed inset-0 z-50">
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="rotation-group-form-title"
           class="absolute right-0 top-0 bottom-0 w-full max-w-[480px] bg-white border-l border-slate-200 shadow-2xl flex flex-col">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 flex-shrink-0 bg-slate-50">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center">
              <IconRefresh :size="16" class="text-blue-600"/>
            </div>
            <div>
              <h2 id="rotation-group-form-title" class="text-slate-800 font-bold text-sm">
          ...
          <button type="button" @click="requestClose" :disabled="saving" aria-label="Fermer"
                  class="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-200 transition disabled:cursor-not-allowed disabled:opacity-50"
          >
            <IconX :size="16"/>
          </button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <div class="field-group">
                ...
                <p class="text-xs text-slate-400 mt-0.5">Étapes avancées à chaque rotation</p>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div class="field-group">
              ...
              <div
                  class="flex items-center justify-between px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg cursor-pointer hover:border-blue-300 transition"
                  @click="form.auto_advance = !form.auto_advance"
              >
                <span class="text-sm font-medium" :class="form.auto_advance ? 'text-blue-600' : 'text-slate-500'">
                  {{ form.auto_advance ? 'Activée' : 'Désactivée' }}
                </span>
                <!-- Toggle inline -->
                <div
                    class="relative w-9 h-5 rounded-full transition-colors duration-200"
                    :class="form.auto_advance ? 'bg-blue-500' : 'bg-slate-300'"
                >
                  <span
              ...
              <p class="text-xs text-slate-400 mt-0.5">Avancement automatique par cron</p>
            </div>
          </div>

          <!-- Preview cycle résumé -->
          <div v-if="form.cycle_length && form.cycle_unit"
               class="flex items-center gap-2.5 p-3 bg-blue-50 border border-blue-100 rounded-xl"
          >
            <IconInfoCircle :size="15" class="text-blue-400 flex-shrink-0"/>
            <p class="text-xs text-blue-700">
              Cycle de <strong>{{ form.cycle_length }} {{
                form.cycle_unit === 'week' ? 'semaine(s)' : 'jour(s)'
              }}</strong>,
          ...
          <!-- Statut -->
          <div
              class="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:border-blue-300 transition"
        ...
        <div
          ...
          <div class="flex items-center gap-2 ">
            ...
            <button @click="submit" :disabled="saving"
                    class="flex items-center gap-1 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-sm font-bold rounded-xl shadow-sm shadow-blue-200 transition disabled:opacity-60"
            >
...
function validate(): boolean {
  ...
  if (!form.cycle_length) {
    errors.cycle_length = 'Champ requis';
    ok = false
  }
  if (!form.start_date) {
