<template>
  <Teleport to="body">
    <div class="fixed inset-0 z-50 flex items-center justify-center p-6" @click.self="$emit('close')">
      ...
      <div class="relative bg-white rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col border border-slate-200 overflow-hidden max-h-[95vh]">
...
function validate(): boolean {
  ...
  req('name')
  if (!form.workday.length) { errors.workday = 'Sélectionnez au moins un jour'; ok = false }
  ...
  if (form.min_working_time && form.normal_session_time && form.max_working_time) {
    ...
    { errors.min_working_time = '≤ durée normale'; ok = false }
    if (form.normal_session_time > form.max_working_time)
    { errors.normal_session_time = '≤ durée max'; ok = false }
  }

  if (form.pause_allowed) {
    if (!form.pause_duration) { errors.pause_duration = 'Requis'; ok = false }
    if (!form.pause_count)    { errors.pause_count    = 'Requis'; ok = false }
  }
...
.field-label { @apply text-[11px] font-semibold text-slate-500 uppercase tracking-wide; }
.field {
