<template>
  <div class="flex flex-col h-full">
    ...
    <div class="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col gap-4">
      ...
      <!-- ══════════════════════════════════════════════════════════ -->
      <div class="hidden md:block bg-white w-full border border-gray-200 overflow-hidden">
      ...
      <div class="md:hidden flex flex-col gap-3">
        ...
        <!-- Loading -->
        <div v-if="loading" class="flex items-center justify-center gap-3 text-gray-400 py-16">
          <IconLoader2 :size="20" class="animate-spin" />
          <span class="text-sm">Chargement...</span>
        </div>

        <!-- Empty -->
        <div v-else-if="items.length === 0" class="flex flex-col items-center gap-3 text-gray-400 py-16">
          <IconShieldCheck :size="40" class="opacity-20" />
          <p class="text-sm">Aucune norme trouvée</p>
          <button @click="openCreate" class="text-xs text-blue-500 hover:text-blue-400 transition">
            Créer la première norme →
        ...
        >
          ...
          <div class="grid grid-cols-3 gap-2 mb-3">
            <div class="bg-gray-50 rounded-lg px-2.5 py-2">
              <p class="text-[10px] text-gray-400 mb-0.5">Min</p>
              <p class="text-xs font-semibold text-emerald-600">{{ fmt(item.min_working_time) }}</p>
            </div>
            <div class="bg-gray-50 rounded-lg px-2.5 py-2">
              <p class="text-[10px] text-gray-400 mb-0.5">Normal</p>
              <p class="text-xs font-semibold text-gray-700">{{ fmt(item.normal_session_time) }}</p>
            </div>
            <div class="bg-gray-50 rounded-lg px-2.5 py-2">
              <p class="text-[10px] text-gray-400 mb-0.5">Max</p>
...
<!--        &lt;!&ndash; Search (déclenché au Enter ou clic sur icône) &ndash;&gt;-->
...
<!--              <td class="px-5 py-3.5">-->
<!--                <p class="text-sm font-semibold text-gray-800">{{ item.name }}</p>-->
<!--&lt;!&ndash;                <p class="text-[10px] text-gray-300 mt-0.5 font-mono">{{ item.guid }}</p>&ndash;&gt;-->
<!--              </td>-->
