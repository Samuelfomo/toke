<template>
  <Teleport to="body">
    <!--
      Backdrop :
      • Mobile  → aligne le panneau en bas (bottom sheet)
      • Desktop → aligne le panneau à droite (drawer)
    -->
    <div
    ...
    >
      ...
      <!--
        Panneau :
        • Mobile  : pleine largeur, max 92dvh depuis le bas, coins arrondis en haut
        • Desktop : 860px de large, hauteur pleine, drawer latéral droit
      -->
      <div
          class="relative bg-white flex flex-col overflow-hidden
                 w-full max-h-[92dvh] rounded-t-2xl
                 sm:w-[860px] sm:max-h-full sm:h-full sm:rounded-none sm:rounded-l-2xl
                 shadow-2xl border border-slate-200"
      >
        <!-- Poignée tactile — mobile seulement -->
        <div class="sm:hidden flex-shrink-0 flex justify-center pt-3 pb-1">
        ...
        <!-- ── Onglets de navigation — mobile seulement ── -->
        <div class="sm:hidden flex-shrink-0 flex border-b border-slate-100">
          <button
        ...
        <div class="flex flex-1 overflow-hidden divide-x divide-slate-100">
          ...
          <!--
            ...
            • Mobile  : visible seulement si onglet 'info' actif
            • Desktop : toujours visible, largeur fixe 288px
          -->
          <div
              class="flex flex-col gap-4 overflow-y-auto px-5 py-4
                     w-full sm:w-72 sm:flex-shrink-0"
              :class="activeTab === 'info' ? 'flex' : 'hidden sm:flex'"
          >
            ...
            <!-- Sur mobile : bouton pour passer à l'onglet horaires -->
            <button
                v-if="form.session_model"
                @click="activeTab = 'schedule'"
                class="sm:hidden w-full py-2 rounded-xl bg-blue-50 text-blue-600 text-xs font-semibold hover:bg-blue-100 transition"
            >
          ...
          <!--
            ...
            • Mobile  : visible seulement si onglet 'schedule' actif
            • Desktop : toujours visible, flex-1
          -->
          <div
              class="flex-col overflow-hidden
                     w-full sm:flex-1"
              :class="activeTab === 'schedule' ? 'flex' : 'hidden sm:flex'"
          >
            ...
            <div class="flex-1 overflow-y-auto px-5 py-3 space-y-2">
              ...
              <div v-if="!form.session_model" class="flex items-center justify-center h-full text-slate-300 text-sm py-12">
                ...
                >
                  ...
                  >
                    <div
                        ...
                        :key="bi"
                        class="flex items-end gap-2 bg-slate-50 rounded-lg px-3 py-2.5 flex-wrap"
                    >
                      <!-- Travail -->
                      <div class="flex flex-col gap-1">
                        <label class="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Travail</label>
                        <div class="flex items-center gap-1.5">
                          <input type="time" v-model="block.work[0]" class="time-field" />
                          <span class="text-slate-300 text-xs">–</span>
                          <input type="time" v-model="block.work[1]" class="time-field" />
                        </div>
                      </div>
...
const activeTab = ref<'info' | 'schedule'>('info')
...
type SessionTemplatePayload = {
  name: string; session_model: string; definition: Record<string, any>
  description?: string; for_rotation?: boolean; default?: boolean; current?: boolean
}
...
async function submit() {
  ...
  const sessionModel = typeof form.session_model === 'string' ? form.session_model : (form.session_model as any).guid
...
<!--      &lt;!&ndash; Modal — très large, aucun scroll &ndash;&gt;-->
...
<!--        &lt;!&ndash; ── Body — 2 colonnes ── &ndash;&gt;-->
<!--        <div class="flex flex-1 overflow-hidden divide-x divide-slate-100">-->

<!--          &lt;!&ndash; COL GAUCHE — infos générales &ndash;&gt;-->
...
<!--              </select>-->
<!--              <p v-if="errors.session_model" class="err">{{ errors.session_model }}</p>-->
<!--              &lt;!&ndash; Info jours autorisés &ndash;&gt;-->
<!--              <div v-if="selectedModel" class="flex flex-wrap gap-1 mt-1">-->
<!--                <span-->
<!--                    v-for="day in selectedModel.workday"-->
<!--                    :key="day"-->
...
<!--            &lt;!&ndash; Info norme requise &ndash;&gt;-->
<!--            <div v-if="!form.session_model"-->
<!--                 class="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-700 text-xs"-->
<!--            >-->
<!--              <IconInfoCircle :size="14" class="flex-shrink-0 mt-0.5" />-->
...
<!--          &lt;!&ndash; COL DROITE — éditeur de définition par jour &ndash;&gt;-->
<!--          <div class="flex-1 flex flex-col overflow-hidden">-->

<!--            &lt;!&ndash; Header éditeur &ndash;&gt;-->
<!--            <div class="px-5 py-3 border-b border-slate-100 flex items-center justify-between flex-shrink-0">-->
<!--              <p class="text-xs font-bold text-slate-700 uppercase tracking-wider">Définition des horaires par jour</p>-->
<!--              <p class="text-[11px] text-slate-400">-->
<!--                {{ activeWorkedDays.length }} jour{{ activeWorkedDays.length > 1 ? 's' : '' }} travaillé{{ activeWorkedDays.length > 1 ? 's' : '' }}-->
...
<!--            &lt;!&ndash; Jours — scroll vertical uniquement ici si nécessaire &ndash;&gt;-->
<!--            <div class="flex-1 overflow-y-auto px-5 py-3 space-y-2">-->
...
<!--                    : 'border-slate-100 bg-slate-50 opacity-50'"-->
<!--                >-->
<!--                  &lt;!&ndash; Day header row &ndash;&gt;-->
<!--                  <div class="flex items-center gap-3 px-4 py-2.5">-->

<!--                    &lt;!&ndash; Nom du jour &ndash;&gt;-->
<!--                    <span class="w-10 text-xs font-bold text-slate-700">{{ day.label }}</span>-->

<!--                    &lt;!&ndash; État du jour : 3 options &ndash;&gt;-->
<!--                    <div class="flex items-center gap-1.5 bg-slate-100 rounded-lg p-0.5">-->
<!--                      <button-->
<!--                          v-for="opt in DAY_STATES"-->
<!--                          :key="opt.value"-->
...
<!--                    &lt;!&ndash; Résumé horaire (si travaillé) &ndash;&gt;-->
<!--                    <span v-if="getDayState(day.value) === 'work' && definition[day.value]?.length"-->
...
<!--                    &lt;!&ndash; Ajouter un bloc (si travaillé) &ndash;&gt;-->
<!--                    <button-->
<!--                        v-if="getDayState(day.value) === 'work'"-->
<!--                        type="button"-->
<!--                        @click="addBlock(day.value)"-->
...
<!--                      &lt;!&ndash; Pause (si norme l'autorise) &ndash;&gt;-->
<!--                      <template v-if="selectedModel?.pause_allowed">-->
<!--                        <div class="flex flex-col gap-1">-->
<!--                          <label class="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Pause</label>-->
<!--                          <div class="flex items-center gap-1.5">-->
