...
export function useGenerationReadiness(
    ...
    const structurallyExcludedGuidSet = computed(
        () =>
            new Set(
                profiles.value
                    .filter(
    ...
    const generationCandidateEmployees = computed(() =>
        activeEmployees.value.filter(
            (employee) => !structurallyExcludedGuidSet.value.has(employee.guid),
        ),
    )

    const includedActiveEmployees = computed(() =>
        generationCandidateEmployees.value.filter(
    ...
    return {
        ...
        candidateEmployees: generationCandidateEmployees,
