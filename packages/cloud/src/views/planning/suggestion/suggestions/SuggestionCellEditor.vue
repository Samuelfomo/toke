<template>
  <PlanningDrawer
      :open="open"
      eyebrow="Modification ponctuelle"
      title="Modifier une journée"
      :description="`${employeeName} · ${formattedDate}`"
      :loading="saving"
      :dirty="isDirty"
      @close="$emit('close')"
  >
    <div class="space-y-5">
      <div class="rounded-2xl border border-amber-100 bg-amber-50/60 p-4">
        <div class="flex gap-3">
          ...
          <div><p class="text-xs font-bold text-amber-950">Modification manuelle</p>
            <p class="mt-1 text-xs leading-5 text-amber-800/80">Cette action cible cette date. Si vous choisissez une garde, sa continuation et le repos post-garde obligatoire sont ajustés automatiquement par l’API. Les autres collaborateurs ne sont pas modifiés.</p></div>
        </div>
      </div>
      <button type="button" class="flex w-full items-start gap-3 rounded-2xl border p-4 text-left"
        ...
        <div><p class="text-xs font-bold text-slate-800">Mettre en repos</p>
          <p class="mt-1 text-xs leading-4 text-slate-500">Aucun Session Template ne sera affecté à cette date.</p>
        </div>
      </button>
      <div><label class="text-xs font-bold text-slate-700">Ou choisir un Session Template</label>
        <div class="mt-3 space-y-2">
          <button v-for="template in compatibleTemplates" :key="template.guid" type="button"
                  ...
                  :class="selectedGuid===template.guid?'border-blue-300 bg-blue-50 ring-2 ring-blue-100':'border-slate-200 bg-white hover:border-slate-300'"
                  @click="selectedGuid=template.guid">
            <div><p class="text-xs font-bold text-slate-800">{{ template.name }}</p>
              <p class="mt-1 text-xs text-slate-500">{{ templateTime(template) }}</p></div>
            <IconCheck v-if="selectedGuid===template.guid" :size="17" class="text-blue-600"/>
          </button>
        </div>
      </div>
    ...
    <template #footer="{ requestClose }">
      <div class="flex items-center justify-between gap-3">
        <button type="button"
                ...
                :disabled="saving" @click="requestClose">Annuler
        </button>
        <button type="button"
...
const {isDirty, markPristine} = useFormDirty(
    () => ({selectedGuid: selectedGuid.value}),
    computed(() => props.open),
);
...
<!--          <IconAlertTriangle :size="18" class="mt-0.5 shrink-0 text-amber-600"/>-->
<!--          <div><p class="text-xs font-bold text-amber-950">Modification manuelle</p>-->
<!--            <p class="mt-1 text-xs leading-5 text-amber-800/80">Cette action change uniquement cette date. Elle ne-->
