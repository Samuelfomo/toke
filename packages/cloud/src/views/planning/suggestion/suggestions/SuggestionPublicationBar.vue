<template>
  <div
      ...
      :class="warning ? 'border-amber-200' : canApprove ? 'border-emerald-200' : 'border-red-200'"
  >
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div class="flex items-start gap-3">
        ...
        >
          <IconAlertTriangle v-if="warning || !canApprove" :size="19" />
          <IconCircleCheck v-else :size="19" />
        </div>
        <div>
          <p class="text-sm font-bold text-slate-900">
            {{ warning
              ? 'Le planning peut être publié avec confirmation'
              : canApprove
                  ? 'La proposition peut être publiée'
      ...
      <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        ...
        <button
            ...
            :class="warning ? 'warning-action' : ''"
            :disabled="loading || !canApprove"
            :title="warning || blocker || 'Valider et publier'"
            @click="$emit('approve')"
        >
          <IconLoader2 v-if="loading" :size="16" class="animate-spin" />
          <IconCircleCheck v-else :size="16" />
          {{ warning ? 'Publier malgré les alertes' : 'Valider et publier' }}
        </button>
      </div>
    </div>
  </div>
...
.primary-button.warning-action { background: #b45309; }
.primary-button.warning-action:hover:not(:disabled) { background: #92400e; }
