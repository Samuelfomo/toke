<template>
  <PlanningDrawer
      ...
      :title="isEdit
            ? 'Modifier le profil de planification'
    ...
    <div class="space-y-6">
      <div class="rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4">
        <div class="flex gap-3">
          <IconInfoCircle
              :size="19"
              class="mt-0.5 shrink-0 text-indigo-600"
          />
          <div>
            ...
            <p class="mt-1 text-xs leading-5 text-indigo-800/80">
              Un profil fixe exige un Session Template. Un profil rotatif ou exclu ne doit pas en recevoir.
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <div class="flex gap-3">
          <IconCalendarPause
              :size="19"
              class="mt-0.5 shrink-0 text-rose-600"
          />
          <div>
            <p class="text-xs font-bold text-rose-950">
              Congé hebdomadaire tournant actif
            </p>
            <p class="mt-1 text-xs leading-5 text-rose-800/80">
              Tous les profils actifs non exclus, y compris les employés FIXED, doivent posséder un ordre unique dans le cycle de congé.
            </p>
          </div>
        </div>
      </div>
      ...
      <section class="space-y-3">
        <div>
          ...
          <p class="mt-0.5 text-[11px] text-slate-400">
            Le collaborateur ne peut plus être changé après la création du profil.
          </p>
        </div>
      ...
      <section>
        ...
        <div class="mt-3 grid gap-3 sm:grid-cols-3">
          <button
              ...
              :class="form.planning_mode === mode.value
                            ? mode.activeClass
          ...
          >
            ...
            <p class="mt-1 text-[10px] leading-4 opacity-75">
              {{ mode.description }}
            </p>
          </button>
        </div>
      ...
      >
        <div class="space-y-2">
          ...
          <p
              v-if="errors.fixed_session_template"
              class="field-error-text"
          >
            {{ errors.fixed_session_template }}
          </p>
          <p class="field-help">
            Le moteur conservera ce même horaire les jours travaillés.
          </p>
        </div>

        <div>
          <label class="text-xs font-bold text-slate-700">
            Gestion du repos fixe
          </label>
          <div class="mt-2 grid gap-2 sm:grid-cols-2">
            ...
            >
              <p class="text-xs font-bold text-slate-800">
                Défini par le template
              </p>
              <p class="mt-1 text-[10px] leading-4 text-slate-500">
                Les jours sans bloc dans le template sont les repos.
              </p>
            </button>
            ...
            >
              ...
              <p class="mt-1 text-[10px] leading-4 text-slate-500">
                L’horaire reste fixe, mais le jour non travaillé peut varier.
              </p>
            </button>
          </div>
        </div>
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <div v-if="form.planning_mode !== 'EXCLUDED'">
          <label class="text-xs font-bold text-slate-700">
            ...
            <span v-if="teamRotationEnabled" class="text-red-500">
                            *
                        </span>
          </label>
          <input
              ...
              :class="{
                            'field-control-error': errors.rotation_order,
                        }"
              placeholder="Ex. 1"
          />
          <p class="field-help">
            {{ teamRotationEnabled
              ? 'Position obligatoire et unique dans le cycle de congé de l’équipe.'
              : 'Repère facultatif pour stabiliser la rotation des collaborateurs.' }}
          </p>
          <p
              v-if="errors.rotation_order"
              class="field-error-text"
          >
        ...
        <div
            :class="form.planning_mode === 'EXCLUDED'
                        ? 'sm:col-span-2'
                        : ''"
        >
          <label class="text-xs font-bold text-slate-700">
          ...
          <div class="relative mt-2">
            ...
            />
            <span class="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] font-semibold text-slate-400">
                            heures
                        </span>
          </div>
          <p class="field-help">
      ...
      <div class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <div>
          ...
          <p class="mt-1 text-[11px] text-slate-500">
            Un profil désactivé ne peut pas être utilisé par le moteur.
          </p>
        </div>
...
const modes = [
  {
    ...
    label: 'Horaire fixe',
    description:
        'Même modèle horaire, avec repos fixe ou variable.',
  ...
  },
  {
    ...
    label: 'Rotation',
    description:
        'Services répartis automatiquement selon les besoins.',
    icon: IconRefresh,
    activeClass:
  ...
  },
  {
    ...
    label: 'Exclu',
    description:
        'Non inclus dans les prochaines générations.',
    icon: IconBan,
    activeClass:
...
const availableEmployees = computed(() =>
    employees.value.filter(
        (employee) =>
            isEdit.value ||
            !props.existingUserGuids.includes(employee.guid),
    ),
)
...
function resetForm(): void {
  ...
  form.planning_mode =
      props.profile?.planning_mode ?? 'ROTATING'
  form.fixed_session_template =
      props.profile?.fixed_session_template?.guid ?? ''
  form.fixed_rest_day_mode =
      props.profile?.fixed_rest_day_mode ?? 'TEMPLATE'
  form.rotation_order =
      props.profile?.rotation_order ?? null
  form.active = props.profile?.active ?? true

  maxWeeklyHours.value = props.profile?.max_weekly_minutes
...
async function loadData(): Promise<void> {
  ...
  try {
    ...
    globalError.value = responseError(
        error,
        'Impossible de charger les collaborateurs, les modèles horaires ou la configuration active.',
    )
  } finally {
...
function validate(): boolean {
  ...
  if (!form.user) {
    errors.user = 'Sélectionnez un collaborateur.'
  }

  if (
      form.planning_mode === 'FIXED' &&
      !form.fixed_session_template
  ) {
    errors.fixed_session_template =
        'Sélectionnez le Session Template fixe.'
  }

  const included =
      form.active && form.planning_mode !== 'EXCLUDED'
  ...
  if (
    ...
    errors.rotation_order =
        'L’ordre est obligatoire avec le congé tournant d’équipe.'
  }

  if (
      form.rotation_order !== null &&
      Number(form.rotation_order) < 1
  ) {
    errors.rotation_order =
  ...
  if (included && form.rotation_order !== null) {
    ...
    if (duplicate) {
      errors.rotation_order =
          `L’ordre ${form.rotation_order} est déjà utilisé par un autre collaborateur.`
    }
  }
...
async function save(): Promise<void> {
  ...
  try {
    const response = isEdit.value
        ? await EmployeePlanningProfileService.update(
            props.profile!.guid,
            common,
        )
    ...
    emit(
        'saved',
        response.data.employee_planning_profile,
    )
  } catch (error: any) {
    globalError.value = responseError(
        error,
