<template>
  <Teleport to="body">
    <div class="fixed inset-0 z-50" @click.self="$emit('close')">
      ...
      <!-- Drawer -->
      <div class="absolute right-0 top-0 bottom-0 w-[540px] bg-white border-l border-gray-200 shadow-2xl flex flex-col">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-gray-400 text-xs">Rotation Assignment</p>
            </div>
          </div>
          ...
          <div v-for="(step, idx) in STEPS" :key="step.id" class="flex items-center flex-1 last:flex-none">
            <div class="flex items-center gap-2 flex-shrink-0">
              ...
              <span
                  ...
                  :class="currentStep === idx ? 'text-violet-600' : currentStep > idx ? 'text-gray-500' : 'text-gray-300'"
              >{{ step.label }}</span>
            </div>
            <div v-if="idx < STEPS.length - 1"
                 class="flex-1 h-px mx-3 transition-colors"
                 :class="currentStep > idx ? 'bg-violet-300' : 'bg-gray-200'" />
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5">
          ...
          <div v-show="isEdit || currentStep === 0" class="space-y-4">
            <div v-if="!isEdit">
              <p class="text-sm font-bold text-gray-700 mb-1">Cible de l'assignation</p>
              <p class="text-xs text-gray-400 mb-4">Choisissez à qui s'applique cette rotation.</p>
            </div>
            ...
            <div v-if="!isEdit" class="grid grid-cols-2 gap-3">
              <button
                  ...
                  :class="form.target_type === t.value ? 'border-violet-400 bg-violet-50' : 'border-gray-200 hover:border-gray-300'"
              >
                <div class="w-10 h-10 rounded-full flex items-center justify-center"
                     :class="form.target_type === t.value ? 'bg-violet-100' : 'bg-gray-100'">
                  <component :is="t.icon" :size="20"
                             :class="form.target_type === t.value ? 'text-violet-600' : 'text-gray-400'" />
                </div>
                <div class="text-center">
                  <p class="text-sm font-bold" :class="form.target_type === t.value ? 'text-violet-700' : 'text-gray-600'">
                    {{ t.label }}
                  </p>
            ...
            <div v-if="isEdit && props.assignment" class="p-3 bg-gray-50 border border-gray-200 rounded-xl flex items-center gap-3">
              ...
              <div>
                <p class="text-sm font-semibold text-gray-800">{{ getRotationTargetName(props.assignment) }}</p>
          ...
          <div v-show="isEdit || currentStep === 1" class="space-y-4">
            <div v-if="!isEdit">
              <p class="text-sm font-bold text-gray-700 mb-1">Groupe de rotation</p>
              <p class="text-xs text-gray-400 mb-4">Sélectionnez la rotation à appliquer et le point de départ dans le cycle.</p>
            </div>
            ...
            <div v-if="selectedRotationGroup" class="p-4 bg-violet-50 border border-violet-100 rounded-xl space-y-3">
              <div class="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p class="text-[10px] text-violet-500">
                    Cycle : {{ selectedRotationGroup.cycle_length }} {{ selectedRotationGroup.cycle_unit === 'day' ? 'jour(s)' : 'semaine(s)' }}
              ...
              <div class="space-y-1.5">
                <p class="text-[10px] text-violet-500 font-bold uppercase tracking-wide">Templates du cycle</p>
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <IconArrowRight v-if="selectedRotationGroup.direction === 'forward'" :size="12" class="text-violet-300" />
                  <IconRotateClockwise v-else :size="12" class="text-violet-300" />
                  <span class="text-[10px] text-violet-400 italic">
            ...
            <div v-if="isEdit"
              ...
              <div>
                <p class="text-sm font-semibold text-gray-700">Statut de l'assignation</p>
                <p class="text-xs text-gray-400">{{ form.active ? 'Assignation active' : 'Assignation inactive' }}</p>
              </div>
              <div class="relative w-10 h-5 rounded-full transition-colors duration-200"
                   :class="form.active ? 'bg-violet-500' : 'bg-gray-300'">
          ...
          <div v-if="!isEdit && currentStep === 2" class="space-y-3">
            ...
            <div class="divide-y divide-gray-100 bg-gray-50 rounded-xl overflow-hidden border border-gray-200">
              ...
              <div class="flex items-center justify-between px-4 py-3">
                ...
                <span class="text-xs font-bold text-gray-700">{{ selectedTargetName }}</span>
              </div>
              <div class="flex items-center justify-between px-4 py-3">
                <span class="text-xs text-gray-500 font-medium">Groupe de rotation</span>
                <span class="text-xs font-bold text-gray-700">{{ selectedRotationGroup?.name ?? '—' }}</span>
              </div>
              <div class="flex items-center justify-between px-4 py-3">
                <span class="text-xs text-gray-500 font-medium">Cycle</span>
              ...
              <div class="flex items-center justify-between px-4 py-3">
                <span class="text-xs text-gray-500 font-medium">Offset (départ)</span>
                <span class="text-xs font-bold text-violet-600">
                  {{ form.offset }} — {{ selectedRotationGroup?.cycle_templates.find((ct: ICycleTemplate) => ct.position === form.offset - 1)?.template_snapshot.name ?? '?' }}
                </span>
              </div>
            </div>
          </div>
...
function validateStep(step: number): boolean {
  ...
  if (step === 0) {
    if (!form.target_guid) { errors.target_guid = 'Sélectionnez une cible'; return false }
  }
  if (step === 1) {
    if (!form.rotation_group_guid) { errors.rotation_group_guid = 'Sélectionnez un groupe de rotation'; return false }
...
function validateAll(): boolean {
  ...
  if (!isEdit.value && !form.rotation_group_guid){ errors.rotation_group_guid = 'Champ requis'; ok = false }
  if (form.offset < 1)                           { errors.offset = 'Offset invalide'; ok = false }
  return ok
}
...
onMounted(() => { loadRotationGroups(); loadTargets() })
</script>
...
.field {
  ...
  placeholder-gray-300 focus:outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 transition;
}
