...
function drawDailyTable(engine: AttendancePdfEngine, rows: AttendancePdfTrendRow[]): void {
    engine.table.draw({
        ...
        columns: [
            ...
            { key: 'expected', title: 'Travail', width: 22, align: 'right', value: (row) => String(row.expected) },
            { key: 'attended', title: 'Prés. obs.', width: 24, align: 'right', value: (row) => String(row.attended) },
            { key: 'present', title: 'À l’heure', width: 22, align: 'right', value: (row) => String(row.present) },
            { key: 'late', title: 'Retards obs.', width: 24, align: 'right', value: (row) => String(row.late) },
            { key: 'absent', title: 'Abs. conf.', width: 22, align: 'right', value: (row) => String(row.absent) },
            ...
            { key: 'netDuration', title: 'Durée nette (h)', width: 29, align: 'right', value: (row) => row.netDurationLabel },
            { key: 'issues', title: 'À examiner', width: 22, align: 'right', value: (row) => String(row.issueCount) },
        ],
        rows,
    });
}
