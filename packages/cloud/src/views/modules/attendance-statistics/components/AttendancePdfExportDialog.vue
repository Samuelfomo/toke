...
<template>
  <Teleport to="body">
    <div v-if="open && overview" class="fixed inset-0 z-[120]" role="presentation">
      <button
          type="button"
      ...
      <section
          ...
          tabindex="-1"
          class="absolute left-1/2 top-1/2 flex max-h-[92vh] w-[min(96vw,72rem)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-lg bg-white shadow-2xl outline-none"
      >
        ...
        <div class="flex-1 overflow-y-auto px-5 py-5 sm:px-6">
            ...
            <div class="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              ...
              >
                ...
                <span v-if="item.disabledReason"
                      class="mt-2 block text-xs font-semibold text-amber-700">{{ item.disabledReason }}</span>
              </button>
            </div>
          </fieldset>
          ...
          <div v-if="draft.mode === 'employee_sheet'" class="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <select id="attendance-pdf-employee" v-model="draft.employeeGuid"
                    class="mt-2 min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
              <option :value="null">Sélectionner un collaborateur</option>
              <option v-for="employee in employees" :key="employee.employeeGuid" :value="employee.employeeGuid">
          ...
          <div v-if="draft.mode === 'issues_only'" class="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
            <label for="attendance-pdf-issue" class="text-sm font-bold text-slate-900">Type d’élément
              (optionnel)</label>
            <select id="attendance-pdf-issue" v-model="draft.issue"
                    class="mt-2 min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200">
              <option :value="null">Tous les éléments à examiner</option>
              <option v-for="summary in issueOptions" :key="summary.issue" :value="summary.issue">
                {{ ATTENDANCE_ISSUE_PRESENTATION[summary.issue].label }} · {{ summary.count }}
              </option>
            </select>
          </div>

          <section v-if="preflight" class="mt-6 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4"
            ...
            <dl class="mt-4 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
              <div class="rounded-lg bg-white px-3 py-2">
                <dt class="text-xs text-slate-500">Période</dt>
                <dd class="font-bold text-slate-900">{{ preflight.volume.periodDays }} jours</dd>
              </div>
              <div class="rounded-lg bg-white px-3 py-2">
                <dt class="text-xs text-slate-500">Équipe / sélection</dt>
                <dd class="font-bold text-slate-900">{{
                    preflight.plan.sections.some((item) => item.section === 'team') ? `${preflight.volume.teamRows} lignes` : 'Non incluse'
                  }}
                </dd>
              </div>
              <div class="rounded-lg bg-white px-3 py-2">
                <dt class="text-xs text-slate-500">Occurrences détaillées</dt>
                <dd class="font-bold text-slate-900">{{ preflight.volume.issueOccurrenceRowsRendered }}</dd>
              </div>
              <div class="rounded-lg bg-white px-3 py-2">
                <dt class="text-xs text-slate-500">Fiches individuelles</dt>
                <dd class="font-bold text-slate-900">{{ preflight.volume.employeeDetailCount }}</dd>
              </div>
            </dl>
            <ul v-if="preflight.notices.length || preflight.contract.validation.warnings.length"
                class="mt-3 space-y-1.5 text-xs leading-5 text-slate-700">
              <li v-for="notice in preflight.notices" :key="notice.code"
                  :class="notice.level === 'warning' ? 'font-semibold text-amber-800' : ''">• {{ notice.message }}
              </li>
              <li v-for="warning in preflight.contract.validation.warnings" :key="warning.code" class="text-slate-600">•
                {{ warning.message }}
              </li>
            </ul>
          </section>

          <p v-if="requestResult.error"
             class="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900"
             role="status">{{ requestResult.error }}</p>
          <p v-if="exportError"
        ...
        <footer
            class="flex flex-col-reverse gap-2 border-t border-slate-200 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <p class="text-xs text-slate-500">Le rapport présente les données actuellement disponibles sans modifier les pointages enregistrés.</p>
          <div class="flex gap-2">
            <button type="button"
                    class="min-h-11 rounded-xl border border-slate-300 px-4 text-sm font-bold text-slate-700 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
                    @click="emit('close')">Annuler
            </button>
            <button type="button"
                    class="min-h-11 rounded-xl bg-indigo-700 px-5 text-sm font-bold text-white hover:bg-indigo-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                    :disabled="!canExport" @click="previewPdf">
