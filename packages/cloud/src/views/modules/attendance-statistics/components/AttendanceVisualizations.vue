...
watch(
  ...
  (daily) => { selectedDate.value = findAttendanceDailyOverview(daily, selectedDate.value)?.date ?? null; },
  { immediate: true },
);
...
<template>
  <section id="attendance-visualizations" aria-labelledby="attendance-visualizations-title">
    <div class="mb-3">
      <p class="text-xs font-bold uppercase tracking-[0.14em] text-indigo-600">Lecture temporelle</p>
      <h2 id="attendance-visualizations-title" class="mt-1 text-lg font-bold text-slate-950">Évolution de la période</h2>
      <p class="mt-1 text-sm text-slate-500">Survolez les points pour comprendre la journée, puis cliquez sur une absence ou un retard pour atteindre directement les employés concernés.</p>
    </div>
    <div class="grid gap-5 2xl:grid-cols-[minmax(0,1.65fr)_minmax(320px,0.75fr)]">
      <AttendanceDailyTrend :daily="overview.daily" :selected-date="selectedDate" @select="selectedDate = $event" @explore="handleTrendExplore" />
