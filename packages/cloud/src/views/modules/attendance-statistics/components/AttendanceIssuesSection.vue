...
<template>
  <section id="attendance-issues" class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm" aria-labelledby="attendance-issues-title">
    <div class="flex flex-col gap-3 px-4 pt-5 sm:flex-row sm:items-start sm:justify-between sm:px-5">
      <div>
        <p class="text-xs font-bold uppercase tracking-[0.14em] text-orange-600">Points à vérifier</p>
        <h2 id="attendance-issues-title" class="mt-1 text-xl font-bold text-slate-950">Éléments à examiner</h2>
        <p class="mt-1 max-w-3xl text-sm text-slate-500">
          Retrouvez ici les situations qui peuvent nécessiter une vérification
          ou une action de votre part.
        </p>
      </div>
      <div class="flex flex-wrap items-center justify-end gap-2 self-start">
        ...
        <button
          type="button"
          class="rounded-lg border border-orange-200 bg-white px-3 py-2 text-xs font-bold text-orange-800 hover:bg-orange-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500"
          @click="emit('export')"
        >
    ...
    <div v-if="issues.length === 0" class="px-5 py-10 text-center">
      ...
      <p class="mt-1 text-sm text-slate-500">
        Aucun élément nécessitant votre attention n’a été détecté pour cette période.
      </p>
    </div>
    ...
    <template v-else>
      ...
      <div class="grid gap-3 border-b border-slate-100 px-4 py-4 sm:grid-cols-3 sm:px-5">
        ...
        <div class="rounded-xl bg-slate-50 px-4 py-3">
          <p class="text-xs font-bold uppercase tracking-wide text-slate-500">Occurrences détectées</p>
          <p class="mt-1 text-2xl font-bold text-slate-950">{{ model.summary.occurrenceCount }}</p>
        </div>
        <div class="rounded-xl bg-slate-50 px-4 py-3">
          <p class="text-xs font-bold uppercase tracking-wide text-slate-500">Employés concernés</p>
