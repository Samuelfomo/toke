...
interface EmployeeColumn {
  label: string;
  align?: 'left' | 'right';
  sortKey?: AttendanceEmployeeSortKey;
}

const columns: EmployeeColumn[] = [
  { sortKey: 'employee_name', label: 'Collaborateur', align: 'left' },
  { sortKey: 'expected_days', label: 'Rotations attendues' },
  { label: 'Rotations couvertes' },
  { sortKey: 'absence_days', label: 'Absences' },
  { sortKey: 'attendance_rate', label: 'Présence %' },
  { label: 'À l’heure / retards' },
  { sortKey: 'punctuality_rate', label: 'Ponctualité %' },
  { label: 'Adoption %' },
  { sortKey: 'issue_count', label: 'Situation' },
];
...
function sortAria(key?: AttendanceEmployeeSortKey): 'ascending' | 'descending' | 'none' | undefined {
  if (!key) return undefined;
  if (props.sort.key !== key) return 'none';
  return props.sort.direction === 'asc' ? 'ascending' : 'descending';
}

function situationLabel(employee: AttendanceEmployeeOverview): string {
  if (employee.issueCount <= 0) return 'Aucun élément signalé';
  return `${employee.issueCount} élément${employee.issueCount === 1 ? '' : 's'} à examiner`;
}
</script>
...
<template>
  <div v-if="employees.length === 0" class="px-5 py-12 text-center">
    ...
    <p class="mt-3 font-semibold text-slate-800">Aucun collaborateur ne correspond aux filtres</p>
  ...
  <template v-else>
    <div class="hidden overflow-x-auto lg:block">
      <table class="min-w-full border-separate border-spacing-0 text-sm">
        <caption class="sr-only">Tableau des statistiques individuelles par collaborateur</caption>
        <thead>
          <tr class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
            <th
              ...
              :aria-sort="sortAria(column.sortKey)"
            >
              <button
                v-if="column.sortKey"
                type="button"
              ...
              >
                {{ column.label }}
                <span class="min-w-3 text-indigo-600" aria-hidden="true">{{ sortIndicator(column.sortKey) }}</span>
              </button>
              <span v-else>{{ column.label }}</span>
            </th>
            <th scope="col" class="border-b border-slate-200 px-3 py-3 text-right font-bold">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="employee in employees" :key="employee.employeeGuid" class="group hover:bg-indigo-50/40">
            <td class="border-b border-slate-100 px-3 py-4">
              <button type="button" class="rounded text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500" @click="emit('view', employee)">
                ...
                <span class="mt-0.5 block max-w-[180px] truncate text-xs text-slate-500">{{ employee.employeeGuid }}</span>
              </button>
            </td>
            <td class="border-b border-slate-100 px-3 py-4 text-right font-medium text-slate-700">{{ employee.rates.employeeWorkingDaysExpected }}</td>
            <td class="border-b border-slate-100 px-3 py-4 text-right font-medium text-slate-700">{{ employee.rates.attendedWorkingDays }}</td>
            <td class="border-b border-slate-100 px-3 py-4 text-right font-semibold text-rose-700">{{ employee.rates.absentWorkingDays }}</td>
            <td class="border-b border-slate-100 px-3 py-4 text-right font-bold text-slate-900">{{ formatPercentage(employee.rates.attendanceRate) }}</td>
            <td class="border-b border-slate-100 px-3 py-4 text-right text-slate-700">
              <span class="font-semibold text-emerald-700">{{ employee.rates.onTimeWorkingDays }}</span>
      ...
      <article v-for="employee in employees" :key="employee.employeeGuid" class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="flex items-start justify-between gap-3">
          ...
          <span
            ...
            :class="employee.issueCount > 0 ? 'bg-orange-100 text-orange-800' : 'bg-emerald-100 text-emerald-800'"
          >
            {{ employee.issueCount > 0 ? `${employee.issueCount} à examiner` : 'RAS signalé' }}
          </span>
        </div>

        <dl class="mt-4 grid grid-cols-2 gap-x-3 gap-y-3 text-sm">
          <div><dt class="text-xs text-slate-500">Rotations attendues</dt><dd class="font-bold text-slate-900">{{ employee.rates.employeeWorkingDaysExpected }}</dd></div>
          <div><dt class="text-xs text-slate-500">Rotations couvertes</dt><dd class="font-bold text-slate-900">{{ employee.rates.attendedWorkingDays }}</dd></div>
