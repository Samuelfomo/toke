...
const dataQualityAcknowledged = ref(false);
...
const resolvedPdfPresentationContext = computed(() =>
    buildAttendancePdfPresentationContext({
      base: {
        ...(props.pdfPresentationContext ?? {}),
        tenantName: props.pdfPresentationContext?.tenantName || userStore.tenantName || undefined,
        managerName: props.pdfPresentationContext?.managerName || props.managerName || userStore.fullName || undefined,
        generatedByName: props.pdfPresentationContext?.generatedByName || userStore.fullName || undefined,
      },
...
const dataQualitySignature = computed(() =>
  page.overview.value ? JSON.stringify(page.overview.value.dataQuality) : null,
);

const qualityAlertActive = computed(() =>
  qualityLevel.value !== null && qualityLevel.value !== 'reliable' && !dataQualityAcknowledged.value,
);

function toggleDataQualityPanel(): void {
  if (qualityLevel.value === null || qualityLevel.value === 'reliable') return;
  dataQualityOpen.value = !dataQualityOpen.value;
  if (dataQualityOpen.value) dataQualityAcknowledged.value = true;
}

watch(
  dataQualitySignature,
  (signature, previousSignature) => {
    if (!signature || qualityLevel.value === 'reliable') {
      dataQualityOpen.value = false;
      dataQualityAcknowledged.value = qualityLevel.value === 'reliable';
      return;
    }

    if (signature !== previousSignature) {
      dataQualityAcknowledged.value = dataQualityOpen.value;
    }
  },
  { immediate: true },
);
  ...
  <div class="min-h-screen bg-gradient-to-r from-[#d0e8f7] via-[#f0e4f5] to-[#d0e8f7] px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
    ...
    <div class="mx-auto max-w-[1600px] space-y-5">
      <AttendanceOverviewHeader
        ...
        :quality-panel-open="dataQualityOpen"
        :quality-alert-active="qualityAlertActive"
        :can-export="page.pageState.value === 'ready' && page.overview.value !== null"
        @toggle-filters="filtersOpen = !filtersOpen"
        @toggle-quality="toggleDataQualityPanel"
        ...
        <template v-else-if="page.pageState.value === 'ready' && page.overview.value">
          <slot name="dashboard" :overview="page.overview.value" :filters="page.filters.value" :refresh="page.refresh">
            <div class="space-y-6">
              ...
              >
                <div v-if="qualityLevel !== 'reliable' && dataQualityOpen" id="attendance-data-quality-panel">
                  <AttendanceDataQualityAlert
                    :quality="page.overview.value.dataQuality"
                    @explore-metric="handleDataQualityMetricExplore"
                  />
                </div>
              </Transition>
