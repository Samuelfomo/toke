<template>
  <div class="flex min-h-screen flex-col bg-slate-100">
    ...
    <main class="relative flex-1 overflow-hidden" style="min-height: calc(100vh - 64px)">
      ...
      <aside
          class="absolute left-3 right-3 top-16 z-20 max-h-[calc(100%-5rem)] overflow-y-auto rounded-2xl border border-white/80 bg-white/95 shadow-xl backdrop-blur sm:left-auto sm:right-4 sm:w-[370px]">
        <div class="p-4 sm:p-5">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <div class="mb-1 flex flex-wrap items-center gap-2">
                ...
                >
                  <span :class="siteData.active ? 'bg-emerald-500' : 'bg-slate-400'"
                        class="mr-1.5 h-1.5 w-1.5 rounded-full"></span>
                  {{ siteData.active ? 'Site actif' : 'Site inactif' }}
                </span>
                <span v-if="siteData"
                      class="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-1 text-[11px] font-medium text-indigo-700 ring-1 ring-indigo-100">
            ...
            <button type="button" @click="goBack"
                    class="flex h-9 w-9 flex-none items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-800"
          ...
          <div v-if="loadError" class="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">
            {{ loadError }}
          </div>
          ...
          <template v-if="siteData">
            <div class="mt-4 grid grid-cols-2 gap-2">
              ...
              <div class="rounded-xl bg-slate-50 p-3">
                ...
                <p class="mt-1 text-sm font-semibold text-slate-800">
                  {{ siteData.geofence_radius ? `${siteData.geofence_radius} m` : '—' }}</p>
              </div>
            </div>
            ...
            <div v-if="searchResultTitle" class="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3">
              <div class="flex items-start gap-2">
                <svg class="mt-0.5 h-4 w-4 flex-none text-amber-600" viewBox="0 0 24 24" fill="none"
                     stroke="currentColor">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p v-if="searchResultDescription" class="mt-0.5 text-xs leading-4 text-slate-600">
                    {{ searchResultDescription }}</p>
                  <p class="mt-1 text-[11px] text-amber-700">La recherche ne modifie pas la localisation enregistrée du
                    site.</p>
                </div>
              </div>
            </div>
            ...
            <div class="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
              <button type="button" @click="focusSite"
                      class="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-3 text-sm font-semibold text-indigo-700 transition hover:bg-indigo-100">
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <button type="button" :disabled="isLocating" @click="locateMe"
                      class="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-wait disabled:opacity-60">
            ...
            <button type="button" @click="goToEdit"
                    class="mt-2 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700">
              <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div
          class="absolute bottom-3 left-3 z-10 rounded-xl border border-white/80 bg-white/90 p-3 shadow-lg backdrop-blur sm:bottom-4 sm:left-4">
        <div class="space-y-1.5 text-[11px] text-slate-600">
          <div class="flex items-center gap-2"><span
              class="h-3 w-5 rounded border-2 border-indigo-600 bg-indigo-200/60"></span><span>Zone enregistrée</span>
          </div>
          <div class="flex items-center gap-2"><span class="h-2.5 w-2.5 rounded-full bg-indigo-600"></span><span>Centre du site</span>
          </div>
          <div v-if="hasUserPosition" class="flex items-center gap-2"><span
              class="h-2.5 w-2.5 rounded-full bg-sky-500"></span><span>Votre position</span></div>
        </div>
      </div>
      ...
      <transition enter-active-class="transition duration-200 ease-out" enter-from-class="translate-y-2 opacity-0"
        ...
        <div v-if="toast.visible"
             :class="toast.type === 'success' ? 'bg-emerald-600' : toast.type === 'warning' ? 'bg-amber-600' : 'bg-red-600'"
             class="absolute bottom-4 left-1/2 z-50 max-w-[calc(100%-2rem)] -translate-x-1/2 rounded-xl px-4 py-3 text-sm font-medium text-white shadow-xl sm:bottom-6">
          {{ toast.message }}
        </div>
      </transition>
    </main>
...
onBeforeUnmount(() => {
  if (toastTimer) clearTimeout(toastTimer);
  try {
    searchControl?.events?.remove?.('resultselect', handleSearchResultSelect);
  } catch {
  }
  try {
    searchControl?.events?.remove?.('load', handleSearchLoad);
  } catch {
  }
  try {
    map?.destroy?.();
