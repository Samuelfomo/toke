  ...
  <div>
    ...
    <div class="page-container">
      ...
      <main class="main-content">
        <h2 class="page-title">Quel module souhaitez-vous gérer?</h2>
        ...
        <div class="cards-grid">
          ...
          >
            <div class="card-icon">
              <component :is="card.icon" :stroke-width="2" :class="card.color" class="icon-size" />
            </div>
            <div class="card-label">{{ card.label }}</div>

            <!-- Badge optionnel pour indiquer les rôles requis -->
            <div v-if="card.roles" class="card-roles">
              <span v-for="role in card.roles" :key="role" class="role-badge">
                {{ role }}
              </span>
            </div>
          </div>
