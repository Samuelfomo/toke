...
export const useTeamStore = defineStore('team', () => {
    ...
    const flash = ref<TeamFlash | null>(null)
    ...
    const setFlash = (value: TeamFlash) => {
        flash.value = value
    }
...
function transformEmployee(emp: any): TeamEmployee {
    ...
    const roleItems = Array.isArray(emp.roles)
        ? emp.roles
        : emp.roles?.items ?? (emp.role ? [emp.role] : [])

    const mainRole = roleItems[0]?.name || 'N/A'
    const isManager = roleItems.some((r: any) =>
