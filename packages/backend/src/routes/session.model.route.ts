...
import { HttpStatus, SessionTemplateValidationUtils } from '@toke/shared';

import { TenantConfig } from '../tools/tenant.config.js';
import { SessionModelService } from '../services/session.model.js';
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result: any = await SessionModelService.list(client, { offset, limit });
...
router.post('/', TenantConfig.authenticate, Ensure.post(), async (req: Request, res: Response) => {
  try {
    ...
    const result: any = await SessionModelService.save(client, req.body);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result: any = await SessionModelService.updated(client, guid as string, req.body);
