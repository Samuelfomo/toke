...
import { RotationAssignmentService } from '../services/rotation.assignment.service.js';
...
router.post('/', TenantConfig.authenticate, Ensure.post(), async (req: Request, res: Response) => {
  try {
    ...
    const result: any = await RotationAssignmentService.saveRotationAssignment(client, req.body);
