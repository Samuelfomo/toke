...
export default class Memos extends MemosModel {
  ...
  getAttachments(): Record<string, any> | undefined {
  ...
  setAttachments(attachments: Record<string, any>): Memos {
  ...
  setAutoGenerated(auto: boolean, reason?: string | null): Memos {
    this.auto_generated = auto;
    if (reason) this.auto_reason = reason;
    return this;
  }
  ...
  async submitMemosForValidation(): Promise<void> {
    if (!this.id) {
  ...
  async approve(validator: number, comments?: string): Promise<void> {
    ...
    const success = await this.processValidation(this.id, validator, 'approved', comments);
    if (success) {
  ...
  async reject(validator: number, comments?: string): Promise<void> {
    ...
    const success = await this.processValidation(this.id, validator, 'rejected', comments);
    if (success) {
  ...
  async addFileAttachment(attachment: Record<string, any>): Promise<void> {
    if (!this.id) {
