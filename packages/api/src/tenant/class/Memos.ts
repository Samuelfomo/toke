...
export default class Memos extends MemosModel {
  ...
  static _listByAuthor(author: number): Promise<Memos[] | null> {
    return new Memos().listByAuthor(author);
  }

  static _listByAuthorAndClientStatus(
    author: number,
    is_manager: boolean,
  ): Promise<Memos[] | null> {
    return new Memos().listByAuthorAndClientStatus(author, is_manager);
  }

  static _listByTarget(target: number): Promise<Memos[] | null> {
    return new Memos().listByTarget(target);
  }

  static _listByTargetAndClientStatus(
    target: number,
    is_manager: boolean,
  ): Promise<Memos[] | null> {
    return new Memos().listByTargetAndClientStatus(target, is_manager);
  }
  ...
  getMemoContent(): MemoContent[] | undefined {
    return this.memo_content;
  }

  getTimeline(): MemoContent[] {
    return this.memo_content || [];
  }

  getLatestMessageContent(): MemoContent | null {
    const timeline = this.getTimeline();
    return timeline.length > 0 ? timeline[timeline.length - 1] : null;
  }

  getMessagesByUserGuid(userGuid: string): MemoContent[] {
    return this.getTimeline().filter((content) => content.user === userGuid);
  }
  ...
  hasUserRespondedAlready(userGuid: string): boolean {
    return this.getMessagesByUserGuid(userGuid).length > 0;
  }
  ...
  setMemoContent(content: MemoContent[]): Memos {
    this.memo_content = content;
    return this;
  }
  ...
  setAutoGenerated(auto: boolean): Memos {
    this.auto_generated = auto;
    return this;
  }

  setIsManager(manager: boolean): Memos {
  ...
  async approve(
    ...
    const success = await this.processValidation(
      this.id,
      validator,
      validatorGuid,
      MemoStatus.APPROVED,
  ...
  async escalate(
    ...
    const success = await this.escalateMemoWithReason(
      this.id,
      escalator_guid,
      new_validator,
      message,
  ...
  /**
   * Ajoute un message à la timeline
   */
  async addMessage(
    userGuid: string,
    message: Message[],
  ...
  async removeFileAttachment(index: number): Promise<void> {
    ...
    if (success && this.memo_content && Array.isArray(this.memo_content)) {
      this.memo_content.splice(index, 1);
    } else {
  ...
  async getResponseTimeHours(): Promise<number> {
    if (!this.id) return 0;
    return await this.getAverageResponseTime(this.id);
  }

  async countUserExchanges(user1Guid: string, user2Guid: string): Promise<number> {
    if (!this.id) return 0;
    return await this.countExchangesBetweenUsers(this.id, user1Guid, user2Guid);
  }
  ...
  async listByAuthor(author: number): Promise<Memos[] | null> {
    const dataset = await this.listAllByAuthor(author);
    if (!dataset || dataset.length === 0) return null;
    return dataset.map((data) => new Memos().hydrate(data));
  }

  async listByAuthorAndClientStatus(author: number, is_manager: boolean): Promise<Memos[] | null> {
    const dataset = await this.listAllByAuthorAndClientStatus(author, is_manager);
    if (!dataset || dataset.length === 0) return null;
    return dataset.map((data) => new Memos().hydrate(data));
  }

  async listByTarget(target: number): Promise<Memos[] | null> {
    const dataset = await this.listAllByTarget(target);
    if (!dataset || dataset.length === 0) return null;
    return dataset.map((data) => new Memos().hydrate(data));
  }

  async listByTargetAndClientStatus(target: number, is_manager: boolean): Promise<Memos[] | null> {
    const dataset = await this.listAllByTargetAndClientStatus(target, is_manager);
    if (!dataset || dataset.length === 0) return null;
    return dataset.map((data) => new Memos().hydrate(data));
  }
  ...
  async findAllSubmit(): Promise<Memos[] | null> {
    const dataset = await this.findSubmitForValidation();
    if (!dataset || dataset.length === 0) return null;
    return dataset.map((data) => new Memos().hydrate(data));
  }
  ...
  async toJSON(view: ViewMode = responseValue.FULL): Promise<object> {
    ...
    const baseData = {
      ...
      [RS.TITLE]: this.title,
      [RS.MEMO_CONTENT]: this.memo_content,
      ...
      [RS.AFFECTED_ENTRIES]: entries
        ...
        : null,
      [RS.MESSAGES_COUNT]: this.getMessagesCount(), // ✅ Info utile
      [RS.LATEST_MESSAGE]: this.getLatestMessageContent(), // ✅ Dernier message
    };
  ...
  private hydrate(data: any): Memos {
    ...
    this.memo_content = data.memo_content;
