...
export default class PlanningSuggestionRequirementModel extends BaseModel {
  ...
  protected async create(): Promise<void> {
    ...
    if (duplicate) {
      throw new Error('An active requirement already exists for this configuration slot');
    }

    if (this.allocation_mode === 'FILL_REMAINING') {
      const existingFill = await this.findFillRemainingByDay(this.config!, this.day_of_week!);

      if (existingFill) {
        throw new Error('An active FILL_REMAINING requirement already exists for this day');
      }
    }
