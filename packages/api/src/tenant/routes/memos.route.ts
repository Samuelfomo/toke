...
router.get('/:guid', Ensure.get(), async (req: Request, res: Response) => {
  try {
    if (!MemosValidationUtils.validateGuid(req.params.guid as string)) {
      return R.handleError(res, HttpStatus.BAD_REQUEST, {
...
router.patch('/:guid/validate', Ensure.patch(), async (req: Request, res: Response) => {
  try {
    ...
    // ========================================================================
    // CORRECTION HORAIRE — si le mémo est auto-généré et a des entries liées
    // ========================================================================

    let correctionResult = null;

    if (memoObj.isAutoGenerated() && memoObj.getAffectedEntriesIds()?.length) {
      correctionResult = await MemoCorrectionService.applyCorrections(
        memoObj,
        validatedData.time_corrections,
      );
    }
  ...
  try {
    ...
    await Promise.all(
      guids.map(async (guid) => {
        ...
        // Empêcher la validation de son propre memo
        // if (memoObj.getAuthorUser() === validatorObj.getId() || !memoObj.isAutoGenerated()) {
        //   return R.handleError(res, HttpStatus.UNAUTHORIZED, {
        //     code: MEMOS_CODES.SELF_VALIDATION_NOT_ALLOWED,
        //     message: MEMOS_ERRORS.SELF_VALIDATION_NOT_ALLOWED,
        //   });
...
router.patch('/:guid/revoke', Ensure.patch(), async (req: Request, res: Response) => {
  try {
    ...
    const validation = validateRevokeMemo(req.body);
    let revokeMessage: Message[] | undefined;
    let timeCorrections: TimeCorrectionInput[] | undefined;

    if (validation.success && validation.data) {
      revokeMessage = validation.data.message;
      timeCorrections = validation.data.time_corrections;
    }

    // =====================================================================
    // CORRECTION HORAIRE — même logique que validate
    // auto si time_corrections absent, manuelle si fourni
    // =====================================================================

    let correctionResult = null;

    if (memoObj.isAutoGenerated() && memoObj.getAffectedEntriesIds()?.length) {
      correctionResult = await MemoCorrectionService.applyCorrections(memoObj, timeCorrections);
    }
...
router.get('/target/:userGuid/list', Ensure.get(), async (req: Request, res: Response) => {
  try {
    if (!MemosValidationUtils.validateGuid(req.params.userGuid as string)) {
      return R.handleError(res, HttpStatus.BAD_REQUEST, {
        code: MEMOS_CODES.INVALID_GUID,
        message: MEMOS_ERRORS.GUID_INVALID,
      });
