...
function shiftDay(day: DayKey, offset: number): DayKey {
  const index = DAY_KEYS.indexOf(day);
  return DAY_KEYS[(index + offset) % DAY_KEYS.length]!;
}

function hasWorkOnDay(template: SessionTemplate, day: DayKey): boolean {
  const blocks = template.getDefinition()?.[day];
  return Array.isArray(blocks) && blocks.length > 0;
}

async function validateRequirementTemplates(requirement: PlanningSuggestionRequirement): Promise<{
  code: string;
  message: string;
} | null> {
  ...
  const mainTemplate = await requirement.getSessionTemplateObj();
  ...
  const continuationTemplate = await requirement.getContinuationTemplateObj();
...
router.get('/config/:configGuid', Ensure.get(), async (req: Request, res: Response) => {
  try {
    const config = await PlanningSuggestionConfig._load(req.params.configGuid as string, true);
    ...
    return R.handleSuccess(res, {
      planning_suggestion_requirements: {
        ...
        items: requirements
          ? await Promise.all(requirements.map((requirement) => requirement.toJSON()))
          : [],
      },
    });
  } catch (error: any) {
...
router.get('/:guid', Ensure.get(), async (req: Request, res: Response) => {
  try {
    const guid = validatePlanningSuggestionRequirementGuid(req.params.guid);

    const requirement = await PlanningSuggestionRequirement._load(guid, true);
    ...
    return R.handleSuccess(res, {
      planning_suggestion_requirement: await requirement.toJSON(),
    });
  } catch (error: any) {
...
router.post('/config/:configGuid', Ensure.post(), async (req: Request, res: Response) => {
  try {
    const config = await PlanningSuggestionConfig._load(req.params.configGuid as string, true);
    ...
    const data = validatePlanningSuggestionRequirementCreation(req.body);

    const mainTemplate = await SessionTemplate._load(data.session_template, true);
    ...
    let continuationTemplateId: number | null = null;

    if (data.service_type === 'GUARD') {
      const continuationTemplate = await SessionTemplate._load(data.continuation_template!, true);
      ...
      continuationTemplateId = continuationTemplate.getId()!;
    }

    const requirement = new PlanningSuggestionRequirement()
    ...
    const templateError = await validateRequirementTemplates(requirement);

    if (templateError) {
      return R.handleError(res, HttpStatus.UNPROCESSABLE_ENTITY, templateError);
    }
...
router.put('/:guid', Ensure.put(), async (req: Request, res: Response) => {
  try {
    ...
    const data = validatePlanningSuggestionRequirementUpdate(req.body);
    ...
    if (data.session_template !== undefined) {
      const template = await SessionTemplate._load(data.session_template, true);
    ...
    if (data.continuation_day_offset !== undefined) {
