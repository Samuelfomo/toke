...
type AttendanceClassificationResult = Omit<
  AttendanceDayResult,
...
export function createAttendanceDay(input: CreateAttendanceDayInput): AttendanceDay {
  ...
  const durationBalance = calculateDurationBalance(
    input.date,
    schedule,
    activity,
    classification.result.expectedWorkMinutes,
...
function classifyAttendanceDay(
...
): { result: AttendanceClassificationResult; issues: AttendanceIssue[] } {
  if (schedule.state === 'UNRESOLVED') {
    return {
      ...
      issues: activity.hasPresence && hasNonCarryOverActivity(date, activity)
  ...
  if (schedule.state === 'REST_DAY') {
    return {
      ...
      issues: activity.hasPresence && hasNonCarryOverActivity(date, activity) ? ['PRESENCE_ON_REST_DAY'] : [],
    };
  }
  ...
  const hasAttendanceActivity =
    activity.hasPresence &&
    hasActivityRelevantToSchedule(date, schedule.expectedBlocks, activity);
...
function normalizeActivity(input: AttendanceDayActivityInput): AttendanceDayActivity {
  ...
  const presenceEvidence = normalizePresenceEvidence(input.presenceEvidence ?? null);
  // Compatibilité progressive : si l'ancien producteur ne fournit pas encore
  // la preuve de présence, le comportement historique basé sur la session reste actif.
  const hasPresence =
    presenceEvidence === null ? hasActivity : presenceEvidence.kind !== 'NONE';
...
function detectActivityIssues(activity: AttendanceDayActivity): AttendanceIssue[] {
  ...
  if (activity.presenceEvidence?.kind === 'CORRECTED') {
    issues.push('CORRECTED_PRESENCE');
  }
...
function normalizePresenceEvidence(
  evidence: AttendanceDayActivityInput['presenceEvidence'] | null,
): AttendanceDayActivity['presenceEvidence'] {
  if (evidence === null || evidence === undefined) return null;

  if (!['DIRECT', 'CORRECTED', 'NONE'].includes(evidence.kind)) {
    throw new AttendanceDayInvariantError('presenceEvidence.kind est invalide');
  }

  if (evidence.kind === 'NONE') {
    if (evidence.occurredAt !== null || evidence.occurredDate !== null) {
      throw new AttendanceDayInvariantError(
        'Une absence de preuve de présence ne peut pas avoir de date/heure',
      );
    }
    return { ...evidence, autoGenerated: false };
  }

  if (evidence.occurredAt === null || evidence.occurredDate === null) {
    throw new AttendanceDayInvariantError(
      'Une preuve de présence doit contenir occurredAt et occurredDate',
    );
  }

  parseBusinessTime(evidence.occurredAt);
  validateActivityDate('presenceEvidence.occurredDate', evidence.occurredDate);

  return evidence;
}

function calculateDurationBalance(
  date: BusinessDate,
  schedule: AttendanceDaySchedule,
  activity: AttendanceDayActivity,
  expectedWorkMinutes: number | null,
  status: AttendanceDayResult['status'],
): Pick<
  AttendanceDayResult,
  | 'attributedWorkMinutes'
  | 'rawDeltaMinutes'
  ...
  | 'extraMaxMinutes'
> {
  const extraPolicy = resolveExtraPolicy(schedule);
  const extraAllowed = extraPolicy.resolved ? extraPolicy.allowed : null;
  const extraMaxMinutes =
    extraPolicy.resolved && extraPolicy.allowed ? extraPolicy.maxMinutes : null;

  // E+/E- n'est calculé que pour une occurrence effectivement couverte.
  // Une activité réelle hors du bloc attendu reste visible dans les durées
  // brutes, mais elle n'est pas arbitrairement rattachée à cette occurrence.
  if (
    expectedWorkMinutes === null ||
    schedule.state !== 'WORK_DAY' ||
  ...
  const occurrenceNetMinutes = calculateOccurrenceNetMinutes(
    date,
    schedule.expectedBlocks,
    activity,
  );

  if (occurrenceNetMinutes === null) {
    return {
      attributedWorkMinutes: null,
      rawDeltaMinutes: null,
      deficitMinutes: null,
  ...
  const rawDeltaMinutes = occurrenceNetMinutes - expectedWorkMinutes;
  const deficitMinutes = rawDeltaMinutes < 0 ? Math.abs(rawDeltaMinutes) : 0;

  if (rawDeltaMinutes <= 0) {
    return {
      attributedWorkMinutes: occurrenceNetMinutes,
      rawDeltaMinutes,
      deficitMinutes,
      creditedExtraMinutes: 0,
      excessBeyondExtraMinutes: 0,
      extraAllowed,
      extraMaxMinutes,
    };
  }

  if (!extraPolicy.resolved) {
    return {
      attributedWorkMinutes: occurrenceNetMinutes,
      rawDeltaMinutes,
      deficitMinutes: 0,
  ...
  if (!extraPolicy.allowed) {
    return {
      attributedWorkMinutes: occurrenceNetMinutes,
      rawDeltaMinutes,
      deficitMinutes: 0,
      creditedExtraMinutes: 0,
      excessBeyondExtraMinutes: rawDeltaMinutes,
      extraAllowed: false,
      extraMaxMinutes: null,
    };
  }

  const creditedExtraMinutes = Math.min(rawDeltaMinutes, extraPolicy.maxMinutes);
  return {
    attributedWorkMinutes: occurrenceNetMinutes,
    rawDeltaMinutes,
    deficitMinutes: 0,
    creditedExtraMinutes,
    excessBeyondExtraMinutes: rawDeltaMinutes - creditedExtraMinutes,
    extraAllowed: true,
    extraMaxMinutes: extraPolicy.maxMinutes,
  };
}

function calculateOccurrenceNetMinutes(
  date: BusinessDate,
  blocks: readonly ExpectedWorkBlock[],
  activity: AttendanceDayActivity,
): number | null {
  if (!activity.hasActivity || !activity.hasPresence) return null;

  const intervals = activity.intervals ?? [];

  // Compatibilité avec les producteurs historiques qui ne fournissent pas
  // encore les durées par intervalle : on conserve l'ancien comportement,
  // mais uniquement si l'activité couvre effectivement le planning.
  if (intervals.length === 0) {
    return hasActivityRelevantToSchedule(date, blocks, activity)
      ? activity.netMinutes
      : null;
  }

  const matched = intervals.filter((interval) =>
    intervalMatchesExpectedBlocks(date, blocks, interval),
  );
...
function intervalMatchesExpectedBlocks(
  date: BusinessDate,
  blocks: readonly ExpectedWorkBlock[],
  interval: AttendanceActivityInterval,
): boolean {
  ...
  if (interval.startDate > date) return false;

  // Règle métier : une rotation est couverte dès qu'une présence valide
  // chevauche réellement au moins une partie de sa plage attendue.
  // L'heure/date de début de la session n'a donc pas besoin d'appartenir au
  // même jour civil : une session commencée avant la rotation peut la couvrir.
...
function resolveExtraPolicy(schedule: AttendanceDaySchedule): AttendanceExtraPolicy {
  if (schedule.state !== 'WORK_DAY' || schedule.extraPolicy === undefined) {
    return { resolved: false, allowed: null, maxMinutes: null };
  }
  return schedule.extraPolicy;
}
...
function hasActivityRelevantToSchedule(
  ...
  // La couverture est fondée sur le chevauchement réel avec un bloc attendu,
  // quelle que soit l'heure/date de début de la session. Une session commencée
  // avant la rotation reste donc une présence si elle couvre effectivement
  // une partie de cette rotation. Une activité totalement hors plage reste
  // conservée comme activité factuelle et produit ACTIVITY_OUTSIDE_EXPECTED_BLOCK.
...
function normalizeActivityIntervals(
  ...
  return intervals.map((interval) => {
    ...
    if (
      interval.attributableNetMinutes !== undefined &&
      interval.attributableNetMinutes !== null &&
      (!Number.isFinite(interval.attributableNetMinutes) || interval.attributableNetMinutes < 0)
    ) {
      throw new AttendanceDayInvariantError(
        'interval.attributableNetMinutes doit être positif, nul ou null',
      );
    }
