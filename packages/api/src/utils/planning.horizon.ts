...
function hasCompleteWeekPolicy(config: PlanningHorizonConfig): boolean {
  ...
  const guardPoolNeedsFullWeek =
    guardTeam.completeWeeksOnly && guardTeam.mode === 'WEEKLY_POOL';
...
export function normalizePlanningHorizon(
  ...
  return {
    ...
    completeWeekExpansionApplied:
      solveFrom !== requestedFrom || solveTo !== requestedTo,
  };
}
...
function causalGuardStart(
  ...
  for (const factor of reason.factors ?? []) {
    const match = factor.match(
      /garde commenc(?:ée|é) le (\d{4}-\d{2}-\d{2})/i,
    );
 ...
 */
export function projectEngineResultToRequestedPeriod<
  T extends ProjectableEngineResult,
>(result: T, horizon: PlanningHorizon): T {
  ...
  const weeklyLeaveGroups = result.diagnostics.weeklyLeaveGroups
    .filter((group) =>
      intersects(
        group.weekFrom,
        group.weekTo,
        horizon.requestedFrom,
        horizon.requestedTo,
      ),
    )
    .map((group) => {
      const leaveByEmployee: Record<string, string[]> = {};
      for (const [employeeGuid, dates] of Object.entries(
        group.leaveByEmployee,
      )) {
  ...
  return {
    ...result,
    items,
    conformityScore: Math.round(
      projectedCoverageScore * 0.75 + fairnessScore * 0.25,
    ),
    diagnostics: {
      ...result.diagnostics,
