...
export default class EmployeeLicense extends EmployeeLicenseModel {
  ...
  /**
   * Exports employee master items with revision information.
   */
  static async exportable(paginationOptions: { offset?: number; limit?: number } = {}): Promise<{
    ...
    const allLicenses = await this._list(
      { ['contractual_status']: ContractualStatus.ACTIVE },
      paginationOptions,
    );
    if (allLicenses) {
  ...
  /**
   * Loads an employee master based on the provided identifier.
   */
  static _load(identifier: any, byGuid: boolean = false): Promise<EmployeeLicense | null> {
    return new EmployeeLicense().load(identifier, byGuid);
  }

  /**
   * Loads an employee master by employee ID.
   */
  ...
  /**
   * Loads an employee master by employee code.
   */
  ...
  static _getBillableCountForLicense(global_license: number): Promise<number> {
   ...
   */
  async load(identifier: any, byGuid: boolean = false): Promise<EmployeeLicense | null> {
