...
router.get('/status/:status', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const transactionsData = await PaymentTransaction._listByStatus(
      transactionStatus,
      paginationOptions,
    );
    if (!transactionsData) {
    ...
    const paymentTransactions = {
      ...
      items:
        (await Promise.all(
          transactionsData.map(async (transaction) => await transaction.toJSON()),
        )) || [],
    };
...
router.get('/payment-method/:payment_method', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const transactionsData = await PaymentTransaction._listByPaymentMethod(
      paymentMethod,
      paginationOptions,
    );
    if (!transactionsData) {
...
router.get('/billing-cycle/:billing_cycle', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const transactionsData = await PaymentTransaction._listByBillingCycle(
      billingCycle,
      paginationOptions,
    );
    if (!transactionsData) {
...
router.get('/currency/:currency', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const transactionsData = await PaymentTransaction._listByCurrency(
      validCurrency,
      paginationOptions,
    );
    if (!transactionsData) {
...
router.get('/amount-range', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const transactionsData = await PaymentTransaction._listByAmountRange(
      minAmount,
      maxAmount,
      currencyType,
      paginationOptions,
...
router.get('/date-range', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const dateFieldType = ['initiated_at', 'completed_at', 'failed_at'].includes(
    ...
    const transactionsData = await PaymentTransaction._listByDateRange(
      startDate,
      endDate,
      dateFieldType,
      paginationOptions,
...
router.delete('/:guid', Ensure.delete(), async (req: Request, res: Response) => {
  try {
    ...
    if (deleted) {
      console.log(
        `✅ Transaction de paiement supprimée: GUID ${validGuid} (${transaction.getPaymentReference()})`,
      );
      R.handleSuccess(res, {
  ...
  try {
    ...
    if (filters.min_amount_usd !== undefined) {
      filteredTransactions = filteredTransactions.filter(
        (transaction) => (transaction.getAmountUsd() || 0) >= filters.min_amount_usd!,
      );
    }

    if (filters.max_amount_usd !== undefined) {
      filteredTransactions = filteredTransactions.filter(
        (transaction) => (transaction.getAmountUsd() || 0) <= filters.max_amount_usd!,
      );
    }
    ...
    const paymentTransactions = {
      ...
      items:
        (await Promise.all(
          filteredTransactions.map(async (transaction) => await transaction.toJSON()),
        )) || [],
    };
