...
router.get('/', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const paginationOptions = {
      ...
      limit: limit
        ? Math.min(parseInt(limit as string), FRAUD_DETECTION_DEFAULTS.PAGINATION.MAX_LIMIT)
...
router.get('/list', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const items = await Promise.all(fraudLogs.map(async (log) => await log.toJSON()));
...
router.get('/tenant/:tenantId', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const items = await Promise.all(fraudLogs.map((log) => log.toJSON()));
...
router.get('/type/:detectionType', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const fraudLogs = await FraudDetectionLog._listByDetectionType(
      detectionType as FraudDetection,
      paginationOptions,
    );
...
router.get('/risk/:riskLevel', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const fraudLogs = await FraudDetectionLog._listByRiskLevel(
