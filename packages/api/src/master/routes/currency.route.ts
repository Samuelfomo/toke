...
router.post('/', Ensure.post(), async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `✅ Devise créée: ${validatedData.code} - ${validatedData.name} (GUID: ${currency.getGuid()})`,
    );
...
router.put('/:guid', Ensure.put(), async (req: Request, res: Response) => {
  try {
    ...
    if (validatedData.decimal_places !== undefined)
      currency.setDecimalPlaces(validatedData.decimal_places);
    if (validatedData.active !== undefined) currency.setActive(validatedData.active);
