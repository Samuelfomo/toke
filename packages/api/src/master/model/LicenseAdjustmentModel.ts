...
export default class LicenseAdjustmentModel extends BaseModel {
  ...
  protected async listAllByPaymentStatus(
    ...
    return await this.listAll({ [this.db.payment_status]: payment_status }, paginationOptions);
  }
  ...
  /**
   * Récupère tous les avenants avec facture envoyée mais non payés pour une master globale
   */
  protected async listAllInvoicedNotPaidGlobalLicense(global_license: number): Promise<any[]> {
  ...
  protected async getFinancialStatsByCurrency(currency_code?: string): Promise<any[]> {
    ...
    results.forEach((adjustment: any) => {
      ...
      if (adjustment[this.db.payment_status] === PaymentTransactionStatus.COMPLETED) {
        stats[currency].completed_amount_local += parseFloat(
          adjustment[this.db.total_amount_local] || 0,
        );
        stats[currency].completed_amount_usd += parseFloat(
          adjustment[this.db.total_amount_usd] || 0,
        );
      } else {
        stats[currency].pending_amount_local += parseFloat(
          adjustment[this.db.total_amount_local] || 0,
        );
    ...
    Object.keys(stats).forEach((currency) => {
      const currencyAdjustments = results.filter(
        (adj: any) => adj[this.db.billing_currency_code] === currency,
      );
      const totalRate = currencyAdjustments.reduce(
        (sum: number, adj: any) => sum + parseFloat(adj[this.db.exchange_rate_used] || 0),
        0,
      );
  ...
  protected async create(): Promise<void> {
    ...
    if (!guid) {
      throw new Error('Failed to generate GUID for master adjustment');
    }
    ...
    console.log(
      `🟢 Avenant de licence créé - Global License: ${this.global_license} | Employees: ${this.employees_added_count} | GUID: ${guid}`,
    );

    if (!lastID) {
      throw new Error('Failed to create master adjustment');
    }
  ...
  protected async update(): Promise<void> {
    ...
    if (this.adjustment_date !== undefined)
      updateData[this.db.adjustment_date] = this.adjustment_date;
    if (this.employees_added_count !== undefined)
      updateData[this.db.employees_added_count] = this.employees_added_count;
    if (this.months_remaining !== undefined)
      updateData[this.db.months_remaining] = this.months_remaining;
    if (this.price_per_employee_usd !== undefined)
      updateData[this.db.price_per_employee_usd] = this.price_per_employee_usd;
    ...
    if (this.payment_due_immediately !== undefined)
      updateData[this.db.payment_due_immediately] = this.payment_due_immediately;
    if (this.invoice_sent_at !== undefined)
      updateData[this.db.invoice_sent_at] = this.invoice_sent_at;
    if (this.payment_completed_at !== undefined)
      updateData[this.db.payment_completed_at] = this.payment_completed_at;

    const affected = await this.updateOne(this.db.tableName, updateData, { [this.db.id]: this.id });
    if (!affected) {
      throw new Error('Failed to update master adjustment');
    }
  }
  ...
  private async validate(): Promise<void> {
    ...
    if (
      this.billing_currency_code &&
      !LicenseAdjustmentDbStructure.validation.validateBillingCurrencyCode(
        this.billing_currency_code,
      )
    ) {
    ...
    if (
      this.payment_status &&
      !LicenseAdjustmentDbStructure.validation.validatePaymentStatus(this.payment_status)
    ) {
      throw new Error('Invalid payment status');
    }

    if (
      this.tax_rules_applied &&
      !LicenseAdjustmentDbStructure.validation.validateTaxRulesApplied(this.tax_rules_applied)
    ) {
