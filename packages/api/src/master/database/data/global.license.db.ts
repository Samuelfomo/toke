...
export const GlobalLicenseDbStructure = {
  ...
  attributes: {
    ...
    guid: {
      type: DataTypes.INTEGER,
      allowNull: false,
      unique: { name: 'unique_global_license_guid', msg: 'Global master GUID must be unique' },
      validate: {
        isInt: true,
        min: 100000,
        max: 999999,
    ...
    next_renewal_date: {
      ...
      comment: 'Next master renewal date',
    },
    ...
    //   set() {
    //     throw new Error(
    //       'Cannot manually set total_seats_purchased - it is automatically computed by PostgreSQL',
    //     );
    //   },
    // },
    license_status: {
      ...
      validate: {
        isIn: {
          ...
          msg: 'Invalid master status',
        },
      },
      comment: 'Current master status',
    },
  } as ModelAttributes,
  options: {
    tableName: tableName.GLOBAL_LICENSE,
    ...
    comment: 'Global master management table',
  ...
  validation: {
    ...
    // Comprehensive validation for master creation
    isValidForCreation: (data: any): boolean => {
      ...
      return (
        ...
        GlobalLicenseDbStructure.validation.validateCurrentPeriodEnd(
          data.current_period_end,
          data.current_period_start,
        ) &&
        GlobalLicenseDbStructure.validation.validateNextRenewalDate(
    ...
    getValidationErrors: (data: any): string[] => {
      ...
      if (
        data.base_price_usd !== undefined &&
        !GlobalLicenseDbStructure.validation.validateBasePriceUsd(data.base_price_usd)
      ) {
        errors.push(
          'Invalid base_price_usd: must be a positive number between 0.01 and 9999999999.99',
        );
      }

      if (
        data.minimum_seats !== undefined &&
        !GlobalLicenseDbStructure.validation.validateMinimumSeats(data.minimum_seats)
      ) {
      ...
      if (
        ...
        errors.push(
          'Invalid next_renewal_date: must be a valid date on or after current_period_end',
        );
      }

      if (
        data.total_seats_purchased !== undefined &&
        !GlobalLicenseDbStructure.validation.validateTotalSeatsPurchased(data.total_seats_purchased)
      ) {
        errors.push('Invalid total_seats_purchased: must be a non-negative integer');
      }

      if (
        data.license_status !== undefined &&
        !GlobalLicenseDbStructure.validation.validateLicenseStatus(data.license_status)
      ) {
        errors.push(
