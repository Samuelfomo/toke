...
export default class BillingCycleModel extends BaseModel {
  ...
  protected async listAllByStatus(
    ...
    return await this.listAll(
      { [this.db.billing_status]: billing_status },
      paginationOptions,
    );
  }
  ...
  protected async create(): Promise<void> {
    ...
    console.log(`🟢 Cycle de facturation créé - Licence: ${this.global_license} | Période: ${this.period_start?.toISOString().split('T')[0]} - ${this.period_end?.toISOString().split('T')[0]} | GUID: ${guid}`);
  ...
  protected async update(): Promise<void> {
    ...
    if (this.base_employee_count !== undefined) updateData[this.db.base_employee_count] = this.base_employee_count;
    if (this.final_employee_count !== undefined) updateData[this.db.final_employee_count] = this.final_employee_count;
    if (this.base_amount_usd !== undefined) updateData[this.db.base_amount_usd] = this.base_amount_usd;
    if (this.adjustments_amount_usd !== undefined) updateData[this.db.adjustments_amount_usd] = this.adjustments_amount_usd;
    ...
    if (this.base_amount_local !== undefined) updateData[this.db.base_amount_local] = this.base_amount_local;
    if (this.adjustments_amount_local !== undefined) updateData[this.db.adjustments_amount_local] = this.adjustments_amount_local;
    ...
    if (this.invoice_generated_at !== undefined) updateData[this.db.invoice_generated_at] = this.invoice_generated_at;
    if (this.payment_due_date !== undefined) updateData[this.db.payment_due_date] = this.payment_due_date;
  ...
  private calculateAmounts(): void {
    ...
    const adjustmentsLocal = this.adjustments_amount_local ||
    ...
    const taxLocal = this.tax_amount_local ||
      Math.round(taxUsd * this.exchange_rate_used * 100) / 100;
