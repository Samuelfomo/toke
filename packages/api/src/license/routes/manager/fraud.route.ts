    ...
    try {
        ...
        if (employeeObj) {
            ...
            // Analyser les patterns d'activité
             activityPatterns = activityHistory?.map(async activity => await activity.toJSON()
            //     ({
        ...
        const alertsByType = employeeAlerts.reduce((acc, alert) => {
        ...
        const riskScore = employeeAlerts.reduce((total, alert) => {
            return total + alert.getSeverityScore();
        }, 0) / employeeAlerts.length;

        const alertsData = await Promise.all(
            employeeAlerts.map(async alert => await alert.toJSON())
        );
...
router.get('/alerts/late-arrivals', Ensure.get(), async (req: Request, res: Response) => {
    try {
        ...
        const suspiciousEmployees = await ActivityMonitoring._listSuspiciousEmployees(paginationOptions);
        ...
        const lateArrivals = [...(suspiciousEmployees || []), ...(lowActivityEmployees || [])]
            .filter(activity => {
                const monitoringDate = activity.getMonitoringDate();
                return monitoringDate &&
                    monitoringDate.toDateString() === today.toDateString() &&
                    (activity.getPunchCount7Days() || 0) < 2; // Moins de 2 pointages = retard potentiel
            });
...
router.get('/alerts/missing-clockouts', Ensure.get(), async (req: Request, res: Response) => {
    try {
        ...
        const suspiciousActivities = await ActivityMonitoring._listSuspiciousEmployees(paginationOptions);
        ...
        const missingClockouts = suspiciousActivities.filter(activity => {
            ...
            return lastPunchDate &&
                ...
                new Date().getTime() - lastPunchDate.getTime() < 2 * 24 * 60 * 60 * 1000; // < 2 jours
        });
        ...
        const missingClockoutsData = await Promise.all(
            missingClockouts.map(async activity => ({
                ...
                recommendations: ['Verify last clockout', 'Contact employee', 'Manual session closure may be needed'],
            }))
        );
        ...
        return R.handleSuccess(res, {
            ...
            urgency_level: missingClockoutsData.length > 10 ? RiskLevel.HIGH : missingClockoutsData.length > 5 ? RiskLevel.MEDIUM : RiskLevel.LOW,
        });
...
router.get('/alerts/system-notifications', Ensure.get(), async (req: Request, res: Response) => {
    try {
        ...
        const criticalAlerts = await FraudDetectionLog._listCriticalUnresolved(undefined, { limit: 10 });
