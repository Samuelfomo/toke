...
router.get('/currency/:currency', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const methodsData = await PaymentMethod._listBySupportedCurrency(validCurrency, paginationOptions);
    if (!methodsData) {
      return R.handleError(res, HttpStatus.NOT_FOUND, {
        code: PAYMENT_METHOD_CODES.PAYMENT_METHOD_NOT_FOUND,
        message: PAYMENT_METHOD_ERRORS.NOT_FOUND,
...
router.get('/bank-transfer', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const paymentMethods = {
      ...
      items: await Promise.all(bankTransferMethods.map(async (method) => await method.toJSON())) || [],
    };
...
router.post('/', Ensure.post(), async (req: Request, res: Response) => {
  try {
    ...
    console.log(`✅ Moyen de paiement créé: ${validatedData.code} - ${validatedData.name} (GUID: ${methodObj.getGuid()})`);
...
router.put('/:guid', Ensure.put(), async (req: Request, res: Response) => {
  try {
    ...
    if (validateData.supported_currencies !== undefined) methodObj.setSupportedCurrencies(validateData.supported_currencies);
    if (validateData.active !== undefined) methodObj.setActive(validateData.active);
    if (validateData.processing_fee_rate !== undefined) methodObj.setProcessingFeeRate(validateData.processing_fee_rate);
    if (validateData.min_amount_usd !== undefined) methodObj.setMinAmountUsd(validateData.min_amount_usd);
    if (validateData.max_amount_usd !== undefined) methodObj.setMaxAmountUsd(validateData.max_amount_usd);
...
router.delete('/:guid', Ensure.delete(), async (req: Request, res: Response) => {
  try {
    ...
    if (deleted) {
      console.log(`✅ Moyen de paiement supprimé: GUID ${validGuid} (${method.getCode()} - ${method.getName()})`);
      R.handleSuccess(res, {
...
router.get('/list', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    if (filters.min_processing_fee_rate !== undefined) {
      filteredMethods = filteredMethods.filter(method =>
        (method.getProcessingFeeRate() || 0) >= filters.min_processing_fee_rate!
      );
    }

    if (filters.max_processing_fee_rate !== undefined) {
      filteredMethods = filteredMethods.filter(method =>
        (method.getProcessingFeeRate() || 0) <= filters.max_processing_fee_rate!
      );
    }
    ...
    const paymentMethods = {
      ...
      items: await Promise.all(filteredMethods.map(async (method) => await method.toJSON())) || [],
    };
