...
/**
 * GET /global-license/:global_license - Lister les cycles par licence globale
 */
router.get('/global-license/:global_license', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const cyclesData = await BillingCycle._listByGlobalLicense(globalLicenseObj.getId()!, paginationOptions);
    if (!cyclesData) {
      return R.handleError(res, HttpStatus.NOT_FOUND, {
...
router.get('/status/:status', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const cycles = {
      ...
      items: cyclesData ? await Promise.all(cyclesData.map(async (cycle) => await cycle.toJSON())) : [],
    };
...
router.post('/', Ensure.post(), async (req: Request, res: Response) => {
  try {
    ...
    if (validatedData.adjustments_amount_usd !== undefined) cycleObj.setAdjustmentsAmountUsd(validatedData.adjustments_amount_usd);
    if (validatedData.tax_amount_usd !== undefined) cycleObj.setTaxAmountUsd(validatedData.tax_amount_usd);
    if (validatedData.adjustments_amount_local !== undefined) cycleObj.setAdjustmentsAmountLocal(validatedData.adjustments_amount_local);
    if (validatedData.tax_amount_local !== undefined) cycleObj.setTaxAmountLocal(validatedData.tax_amount_local);
    if (validatedData.tax_rules_applied !== undefined) cycleObj.setTaxRulesApplied(validatedData.tax_rules_applied);
    if (validatedData.billing_status !== undefined) cycleObj.setBillingStatus(validatedData.billing_status);
    if (validatedData.invoice_generated_at !== undefined) cycleObj.setInvoiceGeneratedAt(validatedData.invoice_generated_at);
    if (validatedData.payment_completed_at !== undefined) cycleObj.setPaymentCompletedAt(validatedData.payment_completed_at);

    await cycleObj.save();

    console.log(`✅ Cycle de facturation créé: License ${validatedData.global_license} - Période ${validatedData.period_start.toISOString().split('T')[0]} à ${validatedData.period_end.toISOString().split('T')[0]} (GUID: ${cycleObj.getGuid()})`);
...
router.put('/:guid', Ensure.put(), async (req: Request, res: Response) => {
  try {
    ...
    if (validateData.base_employee_count !== undefined) cycleObj.setBaseEmployeeCount(validateData.base_employee_count);
    if (validateData.final_employee_count !== undefined) cycleObj.setFinalEmployeeCount(validateData.final_employee_count);
    if (validateData.base_amount_usd !== undefined) cycleObj.setBaseAmountUsd(validateData.base_amount_usd);
    if (validateData.adjustments_amount_usd !== undefined) cycleObj.setAdjustmentsAmountUsd(validateData.adjustments_amount_usd);
    if (validateData.tax_amount_usd !== undefined) cycleObj.setTaxAmountUsd(validateData.tax_amount_usd);
    if (validateData.billing_currency_code !== undefined) cycleObj.setBillingCurrencyCode(validateData.billing_currency_code);
    if (validateData.exchange_rate_used !== undefined) cycleObj.setExchangeRateUsed(validateData.exchange_rate_used);
    if (validateData.base_amount_local !== undefined) cycleObj.setBaseAmountLocal(validateData.base_amount_local);
    if (validateData.adjustments_amount_local !== undefined) cycleObj.setAdjustmentsAmountLocal(validateData.adjustments_amount_local);
    if (validateData.tax_amount_local !== undefined) cycleObj.setTaxAmountLocal(validateData.tax_amount_local);
    if (validateData.tax_rules_applied !== undefined) cycleObj.setTaxRulesApplied(validateData.tax_rules_applied);
    if (validateData.billing_status !== undefined) cycleObj.setBillingStatus(validateData.billing_status);
    if (validateData.payment_due_date !== undefined) cycleObj.setPaymentDueDate(validateData.payment_due_date);
    if (validateData.invoice_generated_at !== undefined) cycleObj.setInvoiceGeneratedAt(validateData.invoice_generated_at);
    if (validateData.payment_completed_at !== undefined) cycleObj.setPaymentCompletedAt(validateData.payment_completed_at);
...
router.delete('/:guid', Ensure.delete(), async (req: Request, res: Response) => {
  try {
    ...
    if (deleted) {
      console.log(`✅ Cycle de facturation supprimé: GUID ${validGuid} (${cycle.getBillingStatus()} - License ${cycle.getGlobalLicense()})`);
      R.handleSuccess(res, {
...
router.get('/list', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    if (filters.billing_currency_code) conditions.billing_currency_code = filters.billing_currency_code;
...
//       return R.handleError(res, HttpStatus.BAD_REQUEST, {
//         code: BILLING_CYCLE_CODES.CURRENCY_CODE_INVALID,
//         message: 'Invalid currency code. Must be a 3-letter ISO code (e.g., USD, EUR, XAF)',
//       });
//     }
//
//     const paginationOptions = paginationSchema.parse(req.query);
