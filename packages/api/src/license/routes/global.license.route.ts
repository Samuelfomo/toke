...
router.get('/billing-cycle/:months', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const licensesData = await GlobalLicense._listByBillingCycle(months as BillingCycle, paginationOptions);
    const licenses = {
...
router.post('/', Ensure.post(), async (req: Request, res: Response) => {
  try {
    ...
    if (validatedData.base_price_usd !== undefined) licenseObj.setBasePriceUsd(validatedData.base_price_usd);
    if (validatedData.minimum_seats !== undefined) licenseObj.setMinimumSeats(validatedData.minimum_seats);
    if (validatedData.total_seats_purchased !== undefined) licenseObj.setTotalSeatsPurchased(validatedData.total_seats_purchased);
    if (validatedData.license_status !== undefined) licenseObj.setLicenseStatus(validatedData.license_status as LicenseStatus);
...
router.put('/:guid', Ensure.put(), async (req: Request, res: Response) => {
  try {
    ...
    if (validateData.minimum_seats !== undefined) licenseObj.setMinimumSeats(validateData.minimum_seats);
    if (validateData.current_period_start !== undefined) licenseObj.setCurrentPeriodStart(validateData.current_period_start);
    if (validateData.current_period_end !== undefined) licenseObj.setCurrentPeriodEnd(validateData.current_period_end);
    if (validateData.next_renewal_date !== undefined) licenseObj.setNextRenewalDate(validateData.next_renewal_date);
    if (validateData.total_seats_purchased !== undefined) licenseObj.setTotalSeatsPurchased(validateData.total_seats_purchased);
    if (validateData.license_status !== undefined) licenseObj.setLicenseStatus(validateData.license_status);
...
router.delete('/:guid', Ensure.delete(), async (req: Request, res: Response) => {
  try {
    ...
    if (deleted) {
      console.log(`✅ Licence globale supprimée: GUID ${validGuid} (${license.getLicenseType()} - Tenant ${license.getTenant()})`);
      R.handleSuccess(res, {
        message: 'Global license deleted successfully',
        guid: validGuid,
