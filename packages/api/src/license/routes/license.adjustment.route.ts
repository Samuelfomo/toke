...
/**
 * GET /global-license/:global_license - Lister les avenants par licence globale
 */
  ...
  try {
    ...
    if (isNaN(globalLicense)) {
      return R.handleError(res, HttpStatus.BAD_REQUEST, {
        ...
        message: 'Global license ID must be a number',
      });
    }

    const paginationOptions = paginationSchema.parse(req.query);

    const adjustmentsData = await LicenseAdjustment._listByGlobalLicense(globalLicense, paginationOptions);
    if (!adjustmentsData) {
    ...
    const licenseAdjustments = {
      ...
      items: await Promise.all(adjustmentsData.map(async (adjustment) => await adjustment.toJSON())) || [],
    };
...
router.get('/payment-status/:status', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const adjustmentsData = await LicenseAdjustment._listByPaymentStatus(paymentStatus, paginationOptions);
    if (!adjustmentsData) {
      return R.handleError(res, HttpStatus.NOT_FOUND, {
        code: LICENSE_ADJUSTMENT_CODES.LICENSE_ADJUSTMENT_NOT_FOUND,
        message: LICENSE_ADJUSTMENT_ERRORS.NOT_FOUND,
...
router.get('/currency/:currency', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const adjustmentsData = await LicenseAdjustment._listByCurrency(validCurrency, paginationOptions);
    if (!adjustmentsData) {
      return R.handleError(res, HttpStatus.NOT_FOUND, {
        code: LICENSE_ADJUSTMENT_CODES.LICENSE_ADJUSTMENT_NOT_FOUND,
        message: LICENSE_ADJUSTMENT_ERRORS.NOT_FOUND,
...
router.get('/date-range/created', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const adjustmentsData = await LicenseAdjustment._listCreatedBetween(startDate, endDate, paginationOptions);
    if (!adjustmentsData) {
      return R.handleError(res, HttpStatus.NOT_FOUND, {
        code: LICENSE_ADJUSTMENT_CODES.LICENSE_ADJUSTMENT_NOT_FOUND,
        message: LICENSE_ADJUSTMENT_ERRORS.NOT_FOUND,
...
router.get('/date-range/paid', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const adjustmentsData = await LicenseAdjustment._listPaidBetween(startDate, endDate, paginationOptions);
    if (!adjustmentsData) {
      return R.handleError(res, HttpStatus.NOT_FOUND, {
        code: LICENSE_ADJUSTMENT_CODES.LICENSE_ADJUSTMENT_NOT_FOUND,
        message: LICENSE_ADJUSTMENT_ERRORS.NOT_FOUND,
...
router.put('/:guid', Ensure.put(), async (req: Request, res: Response) => {
  try {
    ...
    if (validateData.global_license !== undefined) adjustmentObj.setGlobalLicense(validateData.global_license);
    if (validateData.adjustment_date !== undefined) adjustmentObj.setAdjustmentDate(validateData.adjustment_date);
    if (validateData.employees_added_count !== undefined) adjustmentObj.setEmployeesAddedCount(validateData.employees_added_count);
    if (validateData.months_remaining !== undefined) adjustmentObj.setMonthsRemaining(validateData.months_remaining);
    ...
    if (validateData.tax_rules_applied ||
 ...
 */
router.patch('/:guid/update-payment-status', Ensure.patch(), async (req: Request, res: Response) => {
  try {
    const validGuid = LA.validateLicenseAdjustmentGuid(req.params.guid);
...
router.get('/list', Ensure.get(), async (req: Request, res: Response) => {
  try {
    ...
    const licenseAdjustments = {
      ...
      items: await Promise.all(adjustmentEntries.map(async (adjustment) => await adjustment.toJSON())) || [],
    };
