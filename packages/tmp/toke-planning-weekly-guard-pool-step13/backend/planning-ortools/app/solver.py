...
class OrToolsPlanner:
    ...
    def _apply_weekly_leave_policy(self) -> None:
        ...
        # A weekly leave always blocks work on the selected day.
        for employee in self.included:
            for iso in self.dates:
                leave = self.weekly_leave[(employee.guid, iso)]
                self.model.Add(
                    leave + self.work_day[(employee.guid, iso)] <= 1
                )

                # A continuation day can never be counted as weekly leave.
                if employee.mode == "ROTATING":
                    ...
                    if previous_guard_terms:
                        self.model.Add(
                            leave + sum(previous_guard_terms) <= 1
                        )

                    # Plateau policy: post-guard recovery remains distinct from
                    # weekly leave, unless the configuration explicitly merges it.
                    if (
                        ...
                        for offset in range(
                            ...
                            if guard_terms:
                                self.model.Add(
                                    leave + sum(guard_terms) <= 1
                                )

        # FIXED employees work every template work day except the one selected
        # as their team weekly leave.
        for employee in self.fixed_rotating_rest:
            assert employee.fixedTemplate is not None
            for iso in self.dates:
                if not template_has_work(employee.fixedTemplate, iso):
    ...
    def _apply_guard_team_policy(self) -> None:
        ...
        for week_monday, dates in self._week_groups().items():
            ...
            for employee in self.rotating:
                ...
                if policy.requireParticipation:
                    self.model.Add(sum(employee_guard_terms) >= pool)

            self.model.Add(
                sum(pool_terms) == policy.employeesPerWeek
            )
    ...
    def _apply_weekly_rest(self) -> None:
        ...
        # Legacy/generic policy: every employee receives the configured minimum.
        # It is intentionally not used by Pharmacie du Plateau.
        for employee in self.rotating:
                ...
                if selected is None:
                    ...
                    if is_weekly_leave:
                        reasons[employee.guid][iso] = DayReason(
                            ...
                            factors=[
                                "Congé attribué selon l’ordre de rotation de l’équipe",
                                "Une seule personne de l’équipe bénéficie de ce congé sur la semaine",
                            ],
                        )
                    else:
                        reasons[employee.guid][iso] = DayReason(
