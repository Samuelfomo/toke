...
const baseEmployeePlanningProfileSchema = z
  .object({
    ...
    fixed_rest_day_mode:
      z.enum(['TEMPLATE', 'ROTATING']).default('TEMPLATE'),
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      ...
      ctx.addIssue({
        ...
        message:
          'fixed_rest_day_mode ROTATING is only allowed for a FIXED employee',
      });
    }
  });

export const createEmployeePlanningProfileSchema =
  baseEmployeePlanningProfileSchema;

export const updateEmployeePlanningProfileSchema =
  z
    .object({
      planning_mode: z.enum(['FIXED', 'ROTATING', 'EXCLUDED']).optional(),
      fixed_session_template: z.string().trim().min(1).max(255).nullable().optional(),
      fixed_rest_day_mode:
        z.enum(['TEMPLATE', 'ROTATING']).optional(),
      rotation_order: z.number().int().min(1).nullable().optional(),
      max_weekly_minutes: z.number().int().min(1).max(10080).nullable().optional(),
...
export const employeePlanningProfileGuidSchema =
