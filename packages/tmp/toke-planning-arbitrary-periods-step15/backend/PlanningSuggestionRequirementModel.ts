...
export default class PlanningSuggestionRequirementModel extends BaseModel {
  ...
  protected async findBySlot(
    ...
    const candidates = (await this.findAll(this.db.tableName, {
    ...
    return (
      candidates.find((candidate: any) =>
        this.sameEligibilityPolicy(
          candidate?.[this.db.eligibility_policy],
          eligibilityPolicy,
        ),
      ) ?? null
    );
  ...
  protected async listAllByConfig(
    configId: number,
  ...
  protected async create(): Promise<void> {
    this.eligibility_policy = this.normalizeEligibilityPolicy(
    ...
    if (
      ...
      const existingFill =
        await this.findFillRemainingByDay(
          this.config!,
          this.day_of_week!,
          this.eligibility_policy,
        );
    ...
    if (!guid) {
      throw new Error(
        'GUID generation failed for PlanningSuggestionRequirement',
      );
    }
    ...
    const lastID = await this.insertOne(this.db.tableName, {
      ...
      [this.db.continuation_template]:
        this.continuation_template ?? null,
      [this.db.continuation_day_offset]:
    ...
    if (!lastID) {
      throw new Error(
        'PlanningSuggestionRequirement creation failed',
      );
    }
  ...
  protected async update(): Promise<void> {
    if (!this.id) {
      throw new Error(
        'ID required to update PlanningSuggestionRequirement',
      );
    }
    ...
    if (!updated) {
      throw new Error(
        'PlanningSuggestionRequirement update failed',
      );
    }
  }
  ...
  private normalizeEligibilityPolicy(
    ...
    const planningModes = (['FIXED', 'ROTATING'] as const).filter(
      (mode) => requestedModes.includes(mode),
    );
    ...
    return {
      planning_modes:
        planningModes.length > 0
          ? [...planningModes]
          : ['FIXED', 'ROTATING'],
      guard_pool_relation: ['ANY', 'MEMBER', 'NON_MEMBER'].includes(
  ...
  private sameEligibilityPolicy(
    ...
    return (
      normalizedLeft.guard_pool_relation ===
  ...
  private validate(): void {
    ...
    if (this.target_employees < this.min_employees) {
      throw new Error(
        'target_employees must be greater than or equal to min_employees',
      );
    }
    ...
    if (
      ...
      throw new Error(
        'max_employees must be greater than or equal to target_employees',
      );
    }
    ...
    if (
      ...
      throw new Error(
        'credited_minutes must be between 1 and 10080',
      );
    }
    ...
    if (this.allocation_mode === 'EXACT') {
      if (
        ...
        throw new Error(
          'max_employees is required for an EXACT requirement',
        );
      }
    ...
    if (
      ...
      throw new Error(
        'FILL_REMAINING is only allowed for a STANDARD requirement',
      );
    }
    ...
    if (!['ANY', 'MEMBER', 'NON_MEMBER'].includes(
      this.eligibility_policy.guard_pool_relation,
    )) {
    ...
    if (this.service_type === 'GUARD') {
      if (!this.continuation_template) {
        throw new Error(
          'continuation_template is required for a GUARD requirement',
        );
      }

      if (this.continuation_day_offset !== 1) {
        throw new Error(
          'continuation_day_offset must be 1 for a GUARD requirement',
        );
      }
    ...
    if (
      ...
      throw new Error(
        'continuation_template is only allowed for a GUARD requirement',
      );
    }

    if (this.continuation_day_offset !== 0) {
      throw new Error(
        'continuation_day_offset must be 0 for a STANDARD requirement',
      );
    }
  }
}
