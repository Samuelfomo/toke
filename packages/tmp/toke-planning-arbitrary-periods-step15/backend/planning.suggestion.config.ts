...
const serviceScopeModeSchema = z.enum([
...
const serviceScopeSchema = z.object({
...
const baseObject = z.object({
  ...
  guard_team_member_service_access:
    guardMemberServiceAccessSchema.default('ANY_SERVICE'),
  ...
  guard_team_max_consecutive_membership_weeks:
    z.number().int().min(1).max(52).nullable().optional(),
...
function validatePolicy(data: ConfigShape, ctx: z.RefinementCtx): void {
  ...
  if (
    data.guard_team_balance_mode === 'STRICT' &&
