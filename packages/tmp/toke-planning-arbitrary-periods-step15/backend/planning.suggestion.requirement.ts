...
const allocationModeSchema = z.enum([
  'EXACT',
  'RANGE',
  'FILL_REMAINING',
]);
...
const createBaseSchema = z
  .object({
    ...
    allocation_mode:
      allocationModeSchema.default('RANGE'),
...
export const planningSuggestionRequirementGuidSchema = z
  .string()
  .trim()
  .min(1)
  .max(255);
...
export function validatePlanningSuggestionRequirementGuid(data: unknown) {
  ...
  if (!result.success) {
    const error: any = new Error(
      'Invalid planning suggestion requirement GUID',
    );
