...
export type PlanningServiceScopeMode =
  | 'ANY'
  | 'SERVICE_TYPE'
  | 'TEMPLATE'
  | 'REQUIREMENT';
...
export default class PlanningSuggestionConfigModel extends BaseModel {
  ...
  private validate(): void {
    ...
    if (!['ANY', 'MEMBER', 'NON_MEMBER'].includes(
      this.weekly_leave_selector.guard_pool_relation,
    )) {
    ...
    if (
      ...
      throw new Error(
        'weekly_leave_days_per_employee cannot exceed allowed days in EXACT mode',
      );
    }
    if (
      this.weekly_leave_mode === 'PER_ELIGIBLE_EMPLOYEE' &&
      ...
      throw new Error(
        'A weekly leave guard-pool relation requires WEEKLY_POOL',
      );
    }
    if (
      this.weekly_leave_max_employees_per_day !== null &&
      ...
      throw new Error(
        'weekly_leave_max_employees_per_day must be null or greater than 0',
      );
    }
    if (!this.weekly_leave_service_scope) {
      throw new Error('weekly_leave_service_scope is required');
    }
    if (!['ANY', 'SERVICE_TYPE', 'TEMPLATE', 'REQUIREMENT'].includes(
    ...
    if (!['ANY_SERVICE', 'GUARD_ONLY'].includes(
      this.guard_team_member_service_access,
    )) {
    ...
    if (
      ...
      throw new Error(
        'guard team consecutive membership weeks must be between 1 and 52',
      );
    }
