...
router.post('/:manager', Ensure.post(), async (req: Request, res: Response) => {
  try {
    ...
    const config = new PlanningSuggestionConfig()
      ...
      .setWeeklyLeaveDaysPerEmployee(
        data.weekly_leave_days_per_employee,
      )
      .setWeeklyLeaveCountMode(data.weekly_leave_count_mode)
      .setWeeklyLeaveMaxEmployeesPerDay(
        data.weekly_leave_max_employees_per_day ?? null,
      )
      .setWeeklyLeaveRequireWorkOnOtherDays(
        data.weekly_leave_require_work_on_other_days,
      )
      .setWeeklyLeaveServiceScope(
        data.weekly_leave_service_scope,
      )
      ...
      .setGuardTeamEligiblePlanningModes(
        data.guard_team_eligible_planning_modes,
      )
      .setGuardTeamMemberServiceAccess(
        data.guard_team_member_service_access,
      )
      .setGuardTeamBalanceMode(
        data.guard_team_balance_mode,
      )
      .setGuardTeamMaxMembershipSpread(
...
router.put('/:guid', Ensure.put(), async (req: Request, res: Response) => {
  try {
    ...
    if (data.weekly_leave_days_per_employee !== undefined) {
      config.setWeeklyLeaveDaysPerEmployee(
        data.weekly_leave_days_per_employee,
      );
    }
    ...
    if (data.weekly_leave_max_employees_per_day !== undefined) {
      config.setWeeklyLeaveMaxEmployeesPerDay(
        data.weekly_leave_max_employees_per_day ?? null,
      );
    }
    if (data.weekly_leave_require_work_on_other_days !== undefined) {
      config.setWeeklyLeaveRequireWorkOnOtherDays(
        data.weekly_leave_require_work_on_other_days,
      );
    }
    if (data.weekly_leave_service_scope !== undefined) {
      config.setWeeklyLeaveServiceScope(
        data.weekly_leave_service_scope,
      );
    }
    ...
    if (data.guard_team_eligible_planning_modes !== undefined) {
      config.setGuardTeamEligiblePlanningModes(
        data.guard_team_eligible_planning_modes,
      );
    }
    if (data.guard_team_member_service_access !== undefined) {
      config.setGuardTeamMemberServiceAccess(
        data.guard_team_member_service_access,
      );
    }
    ...
    if (data.guard_team_max_membership_spread !== undefined) {
      config.setGuardTeamMaxMembershipSpread(
        data.guard_team_max_membership_spread ?? null,
      );
    }
    if (
      data.guard_team_max_consecutive_membership_weeks !== undefined
    ) {
