...
const baseScheduleExceptionSchema = z.object({
  ...
  groups: z
    .string({
      invalid_type_error: SCHEDULE_EXCEPTION_ERRORS.GROUPS_INVALID,
    })
    .trim()
    .min(SCHEDULE_EXCEPTION_VALIDATION.GROUPS.MIN_LENGTH, SCHEDULE_EXCEPTION_ERRORS.GROUPS_INVALID)
    .max(SCHEDULE_EXCEPTION_VALIDATION.GROUPS.MAX_LENGTH, SCHEDULE_EXCEPTION_ERRORS.GROUPS_INVALID)
    .nullable()
    .optional(),
...
const FIELD_TO_CODE_MAP: Record<string, ScheduleExceptionCode> = {
  ...
  groups: SCHEDULE_EXCEPTION_CODES.GROUPS_INVALID,
