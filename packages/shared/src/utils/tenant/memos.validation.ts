...
export class MemosValidationUtils {
  ...
  /**
   * Validates auto reason
   */
  static validateAutoReason(autoReason: string | null, autoGenerated?: boolean): boolean {
    // Required for auto-generated memos
    if (autoGenerated === true) {
      if (!autoReason || typeof autoReason !== 'string') return false;
      const trimmed = autoReason.trim();
      return (
        trimmed.length >= MEMOS_VALIDATION.AUTO_REASON.MIN_LENGTH &&
        trimmed.length <= MEMOS_VALIDATION.AUTO_REASON.MAX_LENGTH
      );
    }

    // Optional for manual memos
    if (autoReason === null || autoReason === undefined) return true;
    if (typeof autoReason !== 'string') return false;
  ...
  static isValidForCreation(data: any): boolean {
    ...
    return (
      ...
      this.validateAutoReason(data.auto_reason, data.auto_generated) &&
