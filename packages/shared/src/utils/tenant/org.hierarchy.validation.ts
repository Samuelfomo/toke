...
export class OrgHierarchyValidationUtils {
  ...
  static hasOverlappingRelationship(
    ...
    return existingRelationships.some((rel) => {
      ...
      if (rel.subordinate !== subordinateId || rel.supervisor !== supervisorId) return false;
  ...
  static hasCircularHierarchy(
    ...
    const checkCircular = (currentId: number, targetId: number, depth: number): boolean => {
      ...
      const supervisors = hierarchyData
        .filter((rel) => rel.subordinate === currentId && this.isEffective(rel))
        .map((rel) => rel.supervisor);
  ...
  static calculateHierarchyDepth(
    ...
    const calculateDepth = (currentId: number, depth: number): number => {
      ...
      const subordinates = hierarchyData
        .filter((rel) => rel.supervisor === currentId && this.isEffective(rel))
  ...
  static cleanHierarchyData(data: Record<string, any>): Record<string, any> {
    ...
    if (cleaned.subordinate !== undefined && cleaned.subordinate !== null) {
      cleaned.subordinate = Number(cleaned.subordinate);
    }

    if (cleaned.supervisor !== undefined && cleaned.supervisor !== null) {
      cleaned.supervisor = Number(cleaned.supervisor);
    }
  ...
  static isValidForCreation(data: any): boolean {
    ...
    return (
      this.validateSubordinateId(data.subordinate) &&
      this.validateSupervisorId(data.supervisor) &&
      this.validateNotSelfSupervision(data.subordinate, data.supervisor) &&
  ...
  static isValidForUpdate(data: any): boolean {
    ...
    const validations = [
      data.subordinate === undefined || this.validateSubordinateId(data.subordinate),
      data.supervisor === undefined || this.validateSupervisorId(data.supervisor),
    ...
    if (data.subordinate !== undefined && data.supervisor !== undefined) {
      validations.push(this.validateNotSelfSupervision(data.subordinate, data.supervisor));
    }
  ...
  static getValidationErrors(data: any): string[] {
    ...
    if (
      data.subordinate === undefined ||
      data.subordinate === null ||
      !this.validateSubordinateId(data.subordinate)
    ) {
      errors.push(
        `Invalid subordinate: must be between ${ORG_HIERARCHY_VALIDATION.SUBORDINATE.MIN} and ${ORG_HIERARCHY_VALIDATION.SUBORDINATE.MAX}`,
      );
    }
    ...
    if (
      ...
      errors.push(
        `Invalid supervisor: must be between ${ORG_HIERARCHY_VALIDATION.SUPERVISOR.MIN} and ${ORG_HIERARCHY_VALIDATION.SUPERVISOR.MAX}`,
      );
    }
  ...
  static validateFilterData(data: any): boolean {
    return (
      (data.subordinate && this.validateSubordinateId(data.subordinate)) ||
      (data.supervisor && this.validateSupervisorId(data.supervisor)) ||
  ...
  static getSubordinates(
    ...
    return hierarchyData.filter(
      (rel) => rel.supervisor === supervisorId && (includeInactive || this.isEffective(rel)),
    );
  }
  ...
  static getSupervisors(
    ...
    return hierarchyData.filter(
      (rel) => rel.subordinate === subordinateId && (includeInactive || this.isEffective(rel)),
    );
  }
  ...
  static getHierarchyChain(hierarchyData: any[], userId: number, maxLevels: number = 10): any[] {
    ...
    while (level < maxLevels) {
      const supervisor = hierarchyData.find(
        (rel) => rel.subordinate === currentId && this.isEffective(rel),
      );

      if (!supervisor) break;

      chain.push(supervisor);
      currentId = supervisor.supervisor;
      level++;

      // Prevent infinite loops
      if (chain.some((item) => item.supervisor === currentId && item !== supervisor)) {
        break;
      }
    }
  ...
  static getOrganizationTree(hierarchyData: any[], rootUserId: number, maxDepth: number = 10): any {
    const buildTree = (userId: number, depth: number): any => {
      ...
      const subordinates = this.getSubordinates(hierarchyData, userId).map((rel) => ({
        ...rel,
        subordinates: buildTree(rel.subordinate, depth + 1).subordinates,
      }));
  ...
  static getUserRelationshipSummary(
    ...
    const asSubordinate = hierarchyData.filter(
      (rel) => rel.subordinate === userId && this.isEffective(rel),
    );
    const asSupervisor = hierarchyData.filter(
      (rel) => rel.supervisor === userId && this.isEffective(rel),
    );
  ...
  static validateBusinessRules(
    ...
    if (this.hasCircularHierarchy(existingHierarchy, data.subordinate, data.supervisor)) {
      errors.push('Circular hierarchy relationship detected');
    }

    // Check hierarchy depth
    const depth = this.calculateHierarchyDepth(existingHierarchy, data.supervisor);
    if (depth >= maxHierarchyDepth) {
      errors.push(`Hierarchy depth exceeds maximum allowed levels (${maxHierarchyDepth})`);
    }

    // Check span of control
    const spanOfControl = this.calculateSpanOfControl(existingHierarchy, data.supervisor);
